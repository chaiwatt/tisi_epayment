...
class MailBoardAuditorTrackingExaminer extends Mailable
{
    ...
    public function build()
    {
        ...
        if(!empty($this->email_reply)){
           ...
           $mail =     $mail->subject('จัดทำบันทึกข้อความการแต่งตั้งคณะผู้ตรวจประเมิน')
                       ->view('mail.Tracking.mail_board_auditor_examiner')
                       ->with([
                              'title' => $this->title,
                              'data' => $this->data,
                              'export' => $this->export,

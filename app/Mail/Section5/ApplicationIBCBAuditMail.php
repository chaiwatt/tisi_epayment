...
class ApplicationIBCBAuditMail extends Mailable
{
    ...
    public function build()
    {
        ...
        return $this->from(config('mail.from.address'), config('mail.from.name'))
                        ->subject('แจ้งผลการตรวจประเมินผู้ตรวจสอบการทำผลิตภัณฑ์อุตสาหกรรม (IB)')
                        ->view('mail/Section5.application_ibcb_audit')
                        ->with([
                               'subject'          => $subject,
                               'body'             => $body,

...
use App\Models\Cerreport\CertificateHistoryContact;
use App\Http\Controllers\Certify\SendCertificatesController;

class SystemCertificationController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->permission  = str_slug('cerreport-system-certification', '-');
    }

    public function index(Request $request)
    {
        if(auth()->user()->can('view-'.$this->permission)) {
            return view('cerreport.system-certification.index');
        }
        abort(403);
    }
    ...
    public function data_list(Request $request)
    {
        ...
        $export_cb = DB::table((new CertiCBExport)->getTable().' AS export')
                        ->select('export.id', 'export.app_no', 'export.name_standard AS name', 'export.certificate AS certificate_no', 'export.accereditatio_no', 'export.status', 'export.reason_revoke', 'app.tax_id', 
                        'export.date_start', 'export.certificate_path', 'export.certificate_newfile')
                        ->selectRaw('"CB" AS certify_type, "1" AS certificate_type')
                        ->leftJoin((new CertiCb)->getTable().' AS app', 'app.id', '=', 'export.app_certi_cb_id')
                        ->whereIn('export.status', ['4','5']); 
        $export_ib = DB::table((new CertiIBExport)->getTable().' AS export')
                        ->select('export.id', 'export.app_no', 'export.name_unit AS name', 'export.certificate AS certificate_no', 'export.accereditatio_no', 'export.status', 'export.reason_revoke',  'app.tax_id',
                        'export.date_start', 'export.certificate_path', 'export.certificate_newfile')
                        ->selectRaw('"IB" AS certify_type, "2" AS certificate_type')
                        ->leftJoin((new CertiIb)->getTable().' AS app', 'app.id', '=', 'export.app_certi_ib_id')
                        ->whereIn('export.status', ['4','5']); 
        $export_lab =  DB::table((new CertificateExport)->getTable().' AS export')
                        ->select('export.id', 'export.request_number AS app_no', 'export.lab_name AS name', 'certificate_no', 'export.accereditatio_no', 'export.status', 'export.reason_revoke',  'app.tax_id',
                        'export.certificate_date_start AS date_start', 'export.certificate_path', 'export.certificate_newfile')
                        ->selectRaw('"LAB" AS certify_type, app.lab_type AS certificate_type')
                        ->leftJoin((new CertiLab)->getTable().' AS app', 'app.id', '=', 'export.certificate_for')
                        ->whereIn('export.status', ['4','5']); 
                        
        if($filter_certify == '1'){ // หน่วยรับรอง
            $query =  $export_cb->when($filter_search, function ($query, $filter_search){
                                    $search_full = str_replace(' ', '', $filter_search ); 
                                    $query->where(function ($query) use($search_full) {
                                        return $query->where(DB::raw("REPLACE(export.name_standard,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.certificate,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.accereditatio_no,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.app_no,' ','')"), $search_full);
                                    });
                                });
        } else   if($filter_certify == '2'){ // หน่วยตรวจ
            $query =  $export_ib->when($filter_search, function ($query, $filter_search){
                                    $search_full = str_replace(' ', '', $filter_search ); 
                                    $query->where(function ($query) use($search_full) {
                                        return $query->where(DB::raw("REPLACE(export.name_unit,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.certificate,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.accereditatio_no,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.app_no,' ','')"), $search_full);
                                    });
                                });
        } else   if($filter_certify == '3'){ // ทดสอบ สอบเทียบ
            $query =  $export_lab->when($filter_search, function ($query, $filter_search){
                                    $search_full = str_replace(' ', '', $filter_search ); 
                                    $query->where(function ($query) use($search_full) {
                                        return $query->where(DB::raw("REPLACE(export.lab_name,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.certificate_no,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.accereditatio_no,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.request_number,' ','')"), $search_full);
                                    });
                                });
        } else{
            $export_cb =  $export_cb->when($filter_search, function ($query, $filter_search){
                                    $search_full = str_replace(' ', '', $filter_search ); 
                                    $query->where(function ($query) use($search_full) {
                                        return $query->where(DB::raw("REPLACE(export.name_standard,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.certificate,' ','')"), $search_full)
            ...
            $export_ib =  $export_ib->when($filter_search, function ($query, $filter_search){
                                    $search_full = str_replace(' ', '', $filter_search ); 
                                    $query->where(function ($query) use($search_full) {
                                        return $query->where(DB::raw("REPLACE(export.name_unit,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.certificate,' ','')"), $search_full)
            ...
            $export_lab =  $export_lab->when($filter_search, function ($query, $filter_search){
                                    $search_full = str_replace(' ', '', $filter_search ); 
                                    $query->where(function ($query) use($search_full) {
                                        return $query->where(DB::raw("REPLACE(export.lab_name,' ','')"), $search_full)
                                                    ->orWhere(DB::raw("REPLACE(export.certificate_no,' ','')"), $search_full)
        ...
        $certify_types =     ['1' => 'หน่วยรับรอง', '2' => 'หน่วยตรวจ', '3' => 'ห้องปฏิบัติการทดสอบ', '4' => 'ห้องปฏิบัติการสอบเทียบ'];
    ...
    return Datatables::of($query)
                        ...
                        ->addColumn('certify', function ($item) use($certify_types){
                            return  array_key_exists(@$item->certificate_type, $certify_types)?$certify_types[$item->certificate_type]:'N/A';
                        })
                        ->addColumn('certification', function ($item) {
                            $text = '';
                            if(!empty($item->certificate_path) && !empty($item->certificate_newfile) && !empty($item->certificate_no)){
                                $url = url('funtions/get-view').'/'.$item->certificate_path.'/'.$item->certificate_newfile.'/'.$item->certificate_no.'_'.date('Ymd_hms').'.pdf';
                            ...
                            if( $item->status == '5'){
                                $text .= '<div class="text-danger" style="cursor: pointer;"  title="'.$item->reason_revoke.'">ใบรับรองถูกยกเลิก</div>';
                            }
                            return   $text;
                        })
                        ->addColumn('date_start', function ($item){
                            return  HP::revertDateThaiShort($item->date_start, true);
                        })
                        ...
                        ->addColumn('action', function ($item) {
                            ...
                            if(auth()->user()->can('view-'. $this->permission) ) {
                                $action .= '<a href="' .( url('cerreport/system-certification/'.$item->id.'/'.$item->certificate_type) ). '" title="รายละเอียด" class="btn btn-icon btn-circle btn-light-info"> <i class="fa fa-eye"></i></a>';
                            }

                            if(auth()->user()->can('edit-'. $this->permission)) {
                                $action .= ' <a href="' .( url('cerreport/system-certification/'.$item->id.'/'.$item->certificate_type.'/edit') ). '" title="แก้ไข" class="btn btn-icon btn-circle btn-light-warning"> <i class="fa fa-pencil-square-o"></i></a>';
                            }

                            if(auth()->user()->can('delete-'. $this->permission) &&  $item->status == '4') {
                                $action .= ' <span  title="ยกเลิก" class="btn btn-icon btn-circle btn-light-danger btn-cancel"  data-id="'.$item->id.'"   data-certificate_no="'.$item->certificate_no.'"    data-certificate_type="'.$item->certificate_type.'"> <i class="fa fa-trash" aria-hidden="true"></i> </span>';
                            }
    ...
    public function edit($id, $certificate_type=null)
    {
        if(auth()->user()->can('edit-'. $this->permission)) {
            if($certificate_type == '1'){
                $certificate = CertiCBExport::findOrFail($id);
                $certificate->certificate_type = 1;
                $certificate->certificate_id = $id;
            }else if($certificate_type == '2'){
                $certificate = CertiIBExport::findOrFail($id);
                $certificate->certificate_id = $id;
            }else if($certificate_type == '3' || $certificate_type == '4'){
                $certificate = CertificateExport::findOrFail($id);
                $certificate->app_no = @$certificate->request_number;
                $certificate->certificate_type = 3;
                $certificate->certificate_id = $id;
            }else{
                abort(403);
            }
            return view('cerreport.system-certification.edit',compact('certificate'));
        }
        abort(403);
    }

    public function show($id, $certificate_type=null)
    {
        if(auth()->user()->can('view-'. $this->permission)) {
            if($certificate_type == '1'){
                $certificate = CertiCBExport::findOrFail($id);
            ...
            return view('cerreport.system-certification.show',compact('certificate'));
        }
        abort(403);
    }
    ...
    public function GetAddress(Request $request)
    {
        ...
        $data = new stdClass;
        if( in_array($certificate_type, [1]) ){

            $certify = CertiCBExport::find($id);
        ...
        }else if( in_array($certificate_type, [2]) ){

            $certify = CertiIBExport::find($id);
        ...
        }else if( in_array($certificate_type, [3]) ){

            $certify = CertificateExport::find($id);
    ...
    public function update(Request $request, $id)
    {
        if(auth()->user()->can('edit-'.$this->permission)) {
            ...
            if( $type = 'F' ){
                ...
                if($i <= 3){
                    ...
                    if(!empty($DataPdf->file_path)){
                        ...
                        if(!empty($datas->SignatureID)){
                            ...
                            return redirect('cerreport/system-certification')->with('success_message', ('<div>DocumentID:'.$datas->DocumentID.'<div>'.'<div>SignatureID:'.$datas->SignatureID.'<div>'));
                        }else{
                            $i ++;
                            goto start;
                        }
                ...
                return redirect('cerreport/system-certification/'.$send_certify->id.'/edit')->with('success_message', 'เกิดข้อผิดพลาด!');
            }
    ...
    {
        $requestData = $request->except('certificate_type');
        $certificate_type = $request->input('certificate_type');
        try {
            if($certificate_type == '1'){
                $certificate = CertiCBExport::findOrFail($id);
            ...
            // $requestData['ref_id'] = $certificate->id;
            // $requestData['ref_table'] = $certificate->getTable();
            // $requestData['created_by'] = auth()->id();
            // CertificateHistoryContact::create($requestData);
            end:
            $result = (object)['status' => 'success', 'data' => $certificate];
        }catch(\Exception $e) {
            $result = (object)['status' => 'fail', 'message' => $e->getMessage()];
        }
        ...
        if( in_array($cer, [3]) ){ // ห้องปฏิบัติการ LAB
            ...
            if(!is_null($cer_exp_lab)){
                ...
                if(!is_null($certi_lab)){
                    ...
                    // ชื่อภาษาไทย
                    $names         = !empty($cer_exp_lab->lab_name) ?  self::format_data($cer_exp_lab->lab_name,80): [];
    
                    // ชื่อภาษาอังกฤษ
                    $names_en      = !empty($cer_exp_lab->lab_name_en) ?  self::format_data2($cer_exp_lab->lab_name_en,135): [];
                    
                    // ตั้งอยู่เลขที่ Th
                    $address_th    = self::FormatAddress($cer_exp_lab);
                    $address_ths   = !empty($address_th) ?  self::format_data($address_th,75)  : [];

                    // ตั้งอยู่เลขที่ en
                    $address_en   = self::FormatAddressEn($cer_exp_lab);
                    $address_ens  =  !empty($address_en) ?  self::format_data2($address_en,140)  : [];

...
use App\Models\Asurv\EsurvBiss20detail;
use App\Models\Asurv\EsurvVolumeBiss20;
use App\Models\Asurv\EsurvVolumeBiss20Detail;
...
class ReportImportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($filter['filter_title']!=''){
            ...
            $applicant_20ter_ids = EsurvBiss20::where('ref_no', 'LIKE', '%'.$filter['filter_title'].'%')->orWhere('title', 'LIKE', '%'.$filter['filter_title'].'%')->orWhereIn('created_by', $trader_ids)->pluck('id');
            $Query = $Query->whereIn('applicant_20bis_id', $applicant_20ter_ids);
        }


        if($filter['filter_tis']!=''){
            $applicant_20bis_ids = EsurvBiss20::orwhereJsonContains('different_no',$filter['filter_tis'])->select('id');
            $Query = $Query->whereIn('applicant_20bis_id', $applicant_20bis_ids);
        }
        ...
        if($filter['filter_department']!='' || $filter['filter_sub_department']!=''){
            ...
            if(count($tb3_tis_autono) > 0){
                ...
                foreach($tb3_tis_autono as $item){
                        $applicant_20bis_ids = EsurvBiss20::orwhereJsonContains('different_no',explode(" ",$item))->select('id')->get();
                        if(count($applicant_20bis_ids) > 0){
                            foreach($applicant_20bis_ids as $item1){
                                $select[$item1->id] = $item1->id;
                           }
                        }
                }
                $Query = $Query->whereIn('applicant_20bis_id', $select);
            }else{
        ...
        $report_import = $Query->orderby('id','desc')->sortable()->paginate($filter['perPage']);
        $temp_num = $report_import->firstItem();

        //สิทธิ์การตรวจตามกลุ่มงานย่อย
        $user_tis = $user->tis->pluck('tb3_Tisno');

        return view('asurv.report_import.index', compact('report_import', 'filter', 'temp_num','user_tis'));
    }

    public function create()
    {
        return view('asurv.report_import.create');
    }

    public function show($id)
    {
        return view('asurv.report_import.show');
    }

    public function edit($ID)
    {
        $data = EsurvVolumeBiss20::findOrFail($ID);
        ...
        $data_volume_main = EsurvVolumeBiss20::query()->where('applicant_20bis_id',$data->applicant_20bis_id)->get();
        $id_detail = array();
        foreach ($data_volume_main as $list_get_detail){
            $id_detail[] = $list_get_detail->id;
        }
        // $data_detail = EsurvVolumeBiss20Detail::query()->whereIn('volume_20bis_id', $id_detail)->groupBy('detail_id')->get();//รายละเอียด

        $data_detail = EsurvVolumeBiss20Detail::query()->where('volume_20bis_id', $data->id)->get();//รายละเอียด

        foreach($data_detail as $key=>$val){
            $sum = EsurvVolumeBiss20Detail::query()->where('volume_20bis_id', '<', $val->volume_20bis_id)->where('detail_id', $val->detail_id)->sum('quantity');
            $data_detail[$key]['quantity_old'] = $sum;
        }

        $data_detail_ck = EsurvVolumeBiss20Detail::query()->whereIn('volume_20bis_id', $id_detail)->groupBy('volume_20bis_id')->get();//รายละเอียด

        $data_detail_app = EsurvBiss20detail::query()->where('applicant_20bis_id',$ID)->get();
        $data_get = EsurvVolumeBiss20::query()->where('applicant_20bis_id',$data->applicant_20bis_id)->get();
        $id = array();
        foreach ($data_get as $list) {
            $id[] = $list->id;
        }
        $data_volume_detail = DB::table('esurv_volume_20bis_product_details')->whereIn('volume_20bis_id',$id)->get();

        $data_file_check = EsurvVolumeBiss20::query()->where('id',$ID)->first();
        ...
        $esurv_ter21                    = EsurvBiss20::where('id',$data->applicant_20bis_id)->first();
        $signer_options = [];
        ...
        return view('asurv.report_import.edit', ["data" => $data, 'data_detail' => $data_detail
    ...
    {
        return redirect('report_import/report_import');
    }
    public function save_data(Request $request)
    {
        ...
        $data = EsurvVolumeBiss20::findOrFail($request->id);
        ...
        if($data->state_notify_report==0){
            $data2 = EsurvBiss20::findOrFail($data->applicant_20bis_id);
            $data2->state_check = 0;
            $data2->save();
        }

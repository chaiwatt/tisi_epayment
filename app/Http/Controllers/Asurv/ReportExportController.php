...
class ReportExportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($filter['filter_title']!=''){
            ...
            $applicant_20ter_ids = EsurvTers20::where('ref_no', 'LIKE', '%'.$filter['filter_title'].'%')->orWhere('title', 'LIKE', '%'.$filter['filter_title'].'%')->orWhereIn('created_by', $trader_ids)->pluck('id');
            $Query = $Query->whereIn('applicant_20ter_id', $applicant_20ter_ids);
        }

        if($filter['filter_tis']!=''){
            $applicant_20ter_ids = EsurvTers20::orwhereJsonContains('different_no',$filter['filter_tis'])->select('id');
            $Query = $Query->whereIn('applicant_20ter_id', $applicant_20ter_ids);
        }
        ...
        if($filter['filter_department']!='' || $filter['filter_sub_department']!=''){
            ...
            if(count($tb3_tis_autono) > 0){
                ...
                $Query = $Query->whereIn('applicant_20ter_id', $select);
            }else{
        ...
        $report_export = $Query->orderby('id','desc')->sortable()->paginate($filter['perPage']);
        $temp_num = $report_export->firstItem();

        //สิทธิ์การตรวจตามกลุ่มงานย่อย
        $user_tis = $user->tis->pluck('tb3_Tisno');

        return view('asurv.report_export.index', compact('report_export', 'filter', 'temp_num','user_tis'));
    }

    public function create()
    {
        return view('asurv.report_export.create');
    }

    public function show($id)
    {
        return view('asurv.report_export.show');
    }

    public function edit($ID)
    {

        $data = EsurvVolumeTers20::findOrFail($ID);
        ...
        $data_volume_main = EsurvVolumeTers20::query()->where('applicant_20ter_id',$data->applicant_20ter_id)->orderby('id','asc')->get();
        // $id_detail = array();
        // foreach ($data_volume_main as $list_get_detail){
        //     $id_detail[] = $list_get_detail->id;
        // }

        // $data_detail = EsurvVolumeTers20Detail::query()->where('volume_20ter_id', $data->id)->get();//รายละเอียด

        // foreach($data_detail as $key=>$val){
        //     $sum = EsurvVolumeTers20Detail::query()->where('volume_20ter_id', '<', $val->volume_20ter_id)->where('detail_id', $val->detail_id)->sum('quantity');
        //     $data_detail[$key]['quantity_old'] = $sum;
        // }

        // $data_detail     = Applicant20TerProductDetail::where("applicant_20ter_id", $data->applicant_20ter_id)
        //                                                           ->with('data_unit')
        //                                                           ->with('informed')
        //                                                           ->get();//ทั้งหมดที่มี ตามเลขอ้างอิง


        // $data_detail_ck = EsurvVolumeTers20Detail::query()->whereIn('volume_20ter_id', $id_detail)->groupBy('volume_20ter_id')->get();//รายละเอียด

        // $data_detail_app = EsurvTers20detail::query()->where('applicant_20ter_id',$ID)->get();
        // $data_get = EsurvVolumeTers20::query()->where('applicant_20ter_id',$data->applicant_20ter_id)->get();
        // $id = array();
        // foreach ($data_get as $list) {
        //     $id[] = $list->id;
        // }
        // $data_volume_detail = DB::table('esurv_volume_20ter_product_details')->whereIn('volume_20ter_id',$id)->get();

        $data_file_check = EsurvVolumeTers20::query()->where('id',$ID)->first();
        ...
        $esurv_ter20 = EsurvTers20::where('id',$data->applicant_20ter_id)->first();
        $signer_options = [];
        if(!is_null($esurv_ter20)){
            $tis_no = json_decode($esurv_ter20->different_no);
        if(!empty($tis_no) &&  count($tis_no) > 0){
            $tb3_tisno              = Tis::whereIn('tb3_TisAutono',$tis_no)->pluck('tb3_Tisno');
        ...
        $volume_20ter_details = EsurvVolumeTers20Detail::where("volume_20ter_id", $data->id)->pluck('quantity', 'detail_id')->toArray();//ที่ถูกเลือก
        $applicant_20ter_details = EsurvTers20detail::where("applicant_20ter_id", $data->applicant_20ter_id)
                                                                  ->with('data_unit')
                                                                  ->with('informed')
                                                                  ->get();//ทั้งหมดที่มี ตามเลขอ้างอิง


        return view('asurv.report_export.edit',compact('data',
                                                        // 'data_detail',
                                                        // 'data_detail_app',
                                                        // 'data_volume_detail',
                                                        'data_volume_main',
    ...
    {
        return redirect('report_export/report_export');
    }
    ...
    public function save_data(Request $request)
    {
        ...
        $data = EsurvVolumeTers20::findOrFail($request->id);
        ...
        if($data->state_notify_report==0){
            $data2 = EsurvTers20::findOrFail($data->applicant_20ter_id);
            $data2->state_check = 0;
            $data2->save();
        }
    ...
    public function get_khrang_thi($applicant_20ter_id, $created_by){
           $result = EsurvVolumeTers20::where('applicant_20ter_id',$applicant_20ter_id)->where('created_by')->count();
           return $result;
    }

    public function get_signer_position($signer_id) {
        $signer_data = Signer::where('id',$signer_id)->first();
        return response()->json($signer_data);

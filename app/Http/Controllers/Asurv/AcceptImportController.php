...
class AcceptImportController extends Controller
{
    private $attach_path;//ที่เก็บไฟล์แนบ
    public function __construct()
    {
        $this->middleware('auth');
        $this->attach_path = 'esurv_attach/applicant_20bis/';
    }
    ...
    public function index(Request $request)
    {
        ...
        if($filter['filter_department']!='' || $filter['filter_sub_department']!=''){
            ...
            if(count($tb3_tis_autono) > 0){
                ...
                foreach($tb3_tis_autono as $item){
                        $esurv_biss20 = EsurvBiss20::orwhereJsonContains('different_no',explode(" ",$item))->select('id')->get();
                        if(count($esurv_biss20) > 0){
                            foreach($esurv_biss20 as $item1){
        ...
        $accept_import = $Query->orderby('id','desc')->sortable()->paginate($filter['perPage']);
        $temp_num = $accept_import->firstItem();

        return view('asurv.accept_import.index', compact('accept_import', 'filter', 'temp_num'));
    }

    public function create()
    {
        return view('asurv.accept_import.create');
    }

    public function show($id)
    {
        return view('asurv.accept_import.show');
    }

    public function edit($id)
    {
        $data = EsurvBiss20::findOrFail($id);
        ...
        $data_detail = EsurvBiss20detail::query()->where('applicant_20bis_id',$id)->get();

        $applicant_20ter = EsurvBiss20::findOrFail($id);
        ...
        $attach_hiring_book = json_decode($applicant_20ter['attach_hiring_book']);
        ...
        $attach_drawing = json_decode($applicant_20ter['attach_drawing']);
        $attach_drawing = !empty($attach_drawing)?$attach_drawing:(object)['file_name'=>'', 'file_client_name'=>''];

        //ไฟล์แนบ

        $data_file_check = EsurvBiss20::query()->where('id',$id)->first();
        ...
        return view('asurv.accept_import.edit',["data"=>$data,'data_detail'=>$data_detail,'signer_options'=> $signer_options], compact(
            'applicant_20ter',
            'product_details',
            'attach_product_plan',
            'attach_hiring_book',
    ...
    {
        return redirect('accept_import/accept_import');
    }
    ...
    public function detail($ID){
        ...
        $data->start_date = $data->start_date?Carbon::createFromFormat("Y-m-d",$data->start_date)->addYear(543)->formatLocalized('%d/%m/%Y'):null;
        $data->end_date = $data->end_date?Carbon::createFromFormat("Y-m-d",$data->end_date)->addYear(543)->formatLocalized('%d/%m/%Y'):null;
        ...
        return view('asurv.accept_import.detail',["data"=>$data,'data_detail'=>$data_detail], compact(
            'applicant_20ter',
            'product_details',
            'attach_product_plan',
            'attach_hiring_book',
    ...
    public function update_status($ID,$STATE){
        ...
        if ($data->save()){
            ...
            $accept_import = $Query->sortable()->paginate($filter['perPage']);
            $temp_num = $accept_import->firstItem();
    ...
    public function check_api_pid(Request $request)
    {
        ...
        $data  =  EsurvBiss20::findOrFail($request->id);
         ...
         return response()->json([
                                    'message' =>  HP_API_PID::CheckDataApiPid($data,(new EsurvBiss20)->getTable())
                                 ]);
    }

...
use App\Models\Asurv\EsurvBiss21detail;
use App\Models\Asurv\EsurvVolumeBiss21;
use App\Models\Asurv\EsurvVolumeBiss21Detail;
...
use Carbon\Carbon;
use HP;
class Report21ImportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($filter['filter_title']!=''){
            ...
            $applicant_21bis_ids = EsurvBiss21::where('ref_no', 'LIKE', '%'.$filter['filter_title'].'%')->orWhere('title', 'LIKE', '%'.$filter['filter_title'].'%')->orWhereIn('created_by', $trader_ids)->pluck('id');
            $Query = $Query->whereIn('applicant_21bis_id', $applicant_21bis_ids);
        }

        if($filter['filter_tis']!=''){
            $applicant_21bis_ids = EsurvBiss21::orwhereJsonContains('different_no',$filter['filter_tis'])->select('id');
            $Query = $Query->whereIn('applicant_21bis_id', $applicant_21bis_ids);
        }
        ...
        if($filter['filter_department']!='' || $filter['filter_sub_department']!=''){
            ...
            if(count($tb3_tis_autono) > 0){
                ...
                foreach($tb3_tis_autono as $item){
                        $applicant_21bis_ids = EsurvBiss21::orwhereJsonContains('different_no',explode(" ",$item))->select('id')->get();
                        if(count($applicant_21bis_ids) > 0){
                            foreach($applicant_21bis_ids as $item1){
                                $select[$item1->id] = $item1->id;
                           }
                        }
                }
                $Query = $Query->whereIn('applicant_21bis_id', $select);
            }else{
        ...
        $report21_import     =  $Query->orderby('id','desc')->sortable()->paginate($filter['perPage']);
        $temp_num            = $report21_import->firstItem();

        //สิทธิ์การตรวจตามกลุ่มงานย่อย
        $user_tis = $user->tis->pluck('tb3_Tisno');

        return view('asurv.report21_import.index', compact('report21_import', 'filter', 'temp_num','user_tis'));
    }

    public function create()
    {
        return view('asurv.report21_import.create');
    }

    public function show($id)
    {
        return view('asurv.report21_import.show');
    }

    public function edit($id)
    {
        $data = EsurvVolumeBiss21::findOrFail($id);
        ...
        $data_volume_main = EsurvVolumeBiss21::query()->where('applicant_21bis_id',$data->applicant_21bis_id)->get();
        $id_detail = array();
        foreach ($data_volume_main as $list_get_detail){
            $id_detail[] = $list_get_detail->id;
        }
        // $data_detail = EsurvVolumeBiss21Detail::query()->whereIn('volume_21bis_id', $id_detail)->groupBy('detail_id')->get();//รายละเอียด

        $data_detail = EsurvVolumeBiss21Detail::query()->where('volume_21bis_id', $data->id)->get();//รายละเอียด

        foreach($data_detail as $key=>$val){
            $sum = EsurvVolumeBiss21Detail::query()->where('volume_21bis_id', '<', $val->volume_21bis_id)->where('detail_id', $val->detail_id)->sum('quantity');
            $data_detail[$key]['quantity_old'] = $sum;
        }

        $data_detail_ck = EsurvVolumeBiss21Detail::query()->whereIn('volume_21bis_id', $id_detail)->groupBy('volume_21bis_id')->get();//รายละเอียด

        $data_detail_app = EsurvBiss21detail::query()->where('applicant_21bis_id',$id)->get();
        $data_get = EsurvVolumeBiss21::query()->where('applicant_21bis_id',$data->applicant_21bis_id)->get();
        $id = array();
        foreach ($data_get as $list) {
            $id[] = $list->id;
        }
        $data_volume_detail = DB::table('esurv_volume_21bis_product_details')->whereIn('volume_21bis_id',$id)->get();

        $data_file_check = EsurvVolumeBiss21::query()->where('id',$id)->first();
        ...
        $esurv_bis21 = EsurvBiss21::where('id',$data->applicant_21bis_id)->first();
        $signer_options = [];
    //     if(!is_null($esurv_bis21)){
    //         $tis_no = json_decode($esurv_bis21->different_no);
    ...
    //    }
        ...
        return view('asurv.report21_import.edit', ["data" => $data, 'data_detail' => $data_detail
    ...
    {
        return redirect('report21_import/report21_import');
    }
    public function save_data(Request $request)
    {
        ...
        $data = EsurvVolumeBiss21::findOrFail($request->id);
        ...
        if($data->state_notify_report==0){
            $data2 = EsurvBiss21::findOrFail($data->applicant_21bis_id);
            $data2->state_check = 0;
            $data2->save();
        }

...
class Accept21ImportController extends Controller
{
    ...
    public function index(Request $request)
    {
            ...
            if($filter_sub_department != ''){
             ...
             if(count($tb3_tis_autono) > 0){
                 ...
                 foreach($tb3_tis_autono as $item){
                         $esurv_biss21s = EsurvBiss21::orwhereJsonContains('different_no',explode(" ",$item))->select('id')->get();
                         if(count($esurv_biss21s) > 0){
                             foreach($esurv_biss21s as $item1){
        ...
        $accept21_import = $Query->orderby('id','desc')->sortable()->paginate($filter['perPage']);
        $temp_num = $accept21_import->firstItem();

        // //สิทธิ์การตรวจตามกลุ่มงานย่อย
        // $user_tis = $user->tis->pluck('tb3_Tisno');

        return view('asurv.accept21_import.index', compact('accept21_import', 'filter', 'temp_num'));
    }

    public function create()
    {
        return view('asurv.accept21_import.create');
    }

    public function show($id)
    {
        return view('asurv.accept21_import.show');
    }

    public function edit($id)
    {
        $data = EsurvBiss21::findOrFail($id);
        ...
        $data_detail = EsurvBiss21detail::query()->where('applicant_21bis_id',$id)->get();

        $applicant_21ter = EsurvBiss21::findOrFail($id);
        ...
        $data_file_check = EsurvBiss21::query()->where('id',$id)->first();
        ...
        return view('asurv.accept21_import.edit',["data"=>$data,'data_detail'=>$data_detail], compact(
            'applicant_21ter',
            'product_details',
            'attach_product_plan',
            'attach_hiring_book',
    ...
    {
        return redirect('accept21_import/accept21_import');
    }
    ...
    public function detail($ID){
        ...
        return view('asurv.accept21_import.detail',["data"=>$data,'data_detail'=>$data_detail], compact(
            'applicant_21ter',
            'product_details',
            'attach_product_plan',
            'attach_hiring_book',
    ...
    public function update_status($ID,$STATE){
        ...
        if ($data->save()){
            ...
            $accept21_import = $Query->sortable()->paginate($filter['perPage']);
            $temp_num = $accept21_import->firstItem();
    ...
    public function check_api_pid(Request $request)
    {
        ...
        $data  =  EsurvBiss21::findOrFail($request->id);
         ...
         return response()->json([
                                    'message' =>  HP_API_PID::CheckDataApiPid($data,(new EsurvBiss21)->getTable())
                                 ]);
    }

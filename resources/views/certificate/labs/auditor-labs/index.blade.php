...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">แต่งตั้งคณะผู้ตรวจประเมิน (LAB)</h3>

                    <div class="pull-right">

                        {{-- @can('add-'.str_slug('auditorlabs'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/certificate/auditor-labs/create') }}">
                      ...
                      <div class="col-md-6 form-group">
                       ...
                       <div class="col-md-4">
                              {!! Form::select('filter_status_id',
                                 ['-1'=>'ขอความเห็นการแต่งตั้งคณะผู้ตรวจประเมิน','1'=>'เห็นชอบการแต่งตั้งคณะผู้ตรวจประเมิน','2'=>'ไม่เห็นชอบการแต่งตั้งคณะผู้ตรวจประเมิน','3'=>"ยกเลิกแต่งตั้งคณะผู้ตรวจประเมินเรียบร้อยแล้ว"], 
                                null, 
                                ['class' => 'form-control', 
                                'id'=>'filter_status_id',
    ...
    @include('certificate.labs.auditor-labs.modaldelete')
    ...
    <script>
        $(document).ready(function () {
            ...
            @endif
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/certificate/auditor-labs/data_list') !!}',
                    data: function (d) {
           ...
           $(document).on('click', '.modal_delete', function(e) {
              var id  = $(this).data('id');
              var url  = "{{ url('certificate/auditor-labs/update_delete') }}";
               $('#reason_cancel').val('');
               $('#form-modal-delete').prop('action',url+'/'+id);
               $('#form-modal-delete').parsley().reset();
               $('#modalDelete').modal('show');
          }); 

          $('#form-modal-delete').parsley().on('field:validated', function() {
            ...
            .on('form:submit', function() {
                $.ajax({
                        method: "POST",
                        url:  $('#form-modal-delete').prop('action'),
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "reason_cancel": $('#reason_cancel').val()
                        }

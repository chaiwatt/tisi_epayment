        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <h3 class="box-title pull-left">ระบบจัดการไฟล์แนบ</h3>

                    <div class="pull-right">

                        @can('edit-'.str_slug('configs-evidence-groups'))
                        ...
                        @can('add-'.str_slug('configs-evidence-groups'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/config/evidence/group/create') }}">
                                <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                            </a>
                        @endcan

                        @can('delete-'.str_slug('configs-evidence-groups'))
                    ...
                    <p class="text-muted m-b-30 font-13"><em>ระบบจัดการไฟล์แนบ</em></p>
                                ...
                                <div class="col-md-12">
                                    <div id="search-btn" class="panel-collapse collapse">
                                        <div class="white-box" style="display: flex; flex-direction: column;">
                                            ...
                                            <div class="col-md-4">
                                                {!! Form::label('filter_system', 'ระบบงานหลัก:', ['class' => 'col-md-12 control-label label-filter']) !!}
                                                <div class="col-md-12">
                                                    {!! Form::select('filter_system', App\Models\Config\ConfigsEvidenceSystem::pluck('title', 'id'), null, ['class' => 'form-control', 'id' => 'filter_system', 'placeholder'=>'-เลือกระบบงานหลัก-' ]); !!}
                                                </div>
                                            </div>
                            ...
                            <table class="table table-striped" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">ระบบงานหลัก</th>
                                        <th class="text-center">ระบบที่นำไปใช้/Url</th>
                                        <th class="text-center">ชื่อไฟล์แนบ</th>
    ...
    <script>
        ...
        $(function () {
            ...
            var table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/config/evidence/group/data_list') !!}',
                    data: function (d) {
            ...
            $('#myTable tbody').on('change', '.js-switch', function(){
                ...
                if ($(this).is(":checked")) {
                    $.ajax({
                        method: "POST",
                        url: "{{ url('config/evidence/group/update_status') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_status": id_status,
                            "state":1
            ...
            $(document).on('click', '#bulk_delete', function(){
                ...
                if(id.length > 0){
                    ...
                    if(confirm("ยืนยันการลบข้อมูล " + id.length + " แถว นี้ ?")){
                        $.ajax({
                            type:"POST",
                            url:  "{{ url('/config/evidence/group/delete') }}",
                            data:{
                                _token: "{{ csrf_token() }}",
                                id: id
                            },
            ...
            $('#publish, #no_publish').on('click', function () {
                    ...
                    if (arrRowId.length > 0) {
                        ...
                        if (confirm("ยืนยันการ"+text_alert+" " + arrRowId.length + " แถว นี้ ?")) {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ url('/config/evidence/group/update_publish') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "id_publish": arrRowId,
                                    "state": state

        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <h3 class="box-title pull-left">ระบบงานหลัก</h3>

                    <div class="pull-right">

                        @can('edit-'.str_slug('configs-evidence-systems'))
                        ...
                        @can('add-'.str_slug('configs-evidence-systems'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/config/evidence/system/create') }}">
                                <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                            </a>
                        @endcan

                        @can('delete-'.str_slug('configs-evidence-systems'))
                    ...
                    <p class="text-muted m-b-30 font-13"><em>ระบบงานหลัก</em></p>
                            ...
                            <table class="table table-striped" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">กลุ่มงานเอกสาร</th>
    ...
    <script>
        ...
        $(function () {
            ...
            var table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/config/evidence/system/data_list') !!}',
                    data: function (d) {
            ...
            $('#myTable tbody').on('change', '.js-switch', function(){
                ...
                if ($(this).is(":checked")) {
                    $.ajax({
                        method: "POST",
                        url: "{{ url('config/evidence/system/update_status') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_status": id_status,
                            "state":1
            ...
            $(document).on('click', '#bulk_delete', function(){
                ...
                if(id.length > 0){
                    ...
                    if(confirm("ยืนยันการลบข้อมูล " + id.length + " แถว นี้ ?")){
                        $.ajax({
                            type:"POST",
                            url:  "{{ url('/config/evidence/system/delete') }}",
                            data:{
                                _token: "{{ csrf_token() }}",
                                id: id
                            },
            ...
            $('#publish, #no_publish').on('click', function () {
                    ...
                    if (arrRowId.length > 0) {
                        ...
                        if (confirm("ยืนยันการ"+text_alert+" " + arrRowId.length + " แถว นี้ ?")) {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ url('/config/evidence/system/update_publish') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "id_publish": arrRowId,
                                    "state": state

	...
	*/
    ...
    You could also use a data-* attribute and content for this. That way "bloats" the HTML, this way means you need to keep HTML and CSS in sync. Lea Verou has a clever way to handle with text-shadow.
		*/
		...
		td:nth-of-type(2):before { content: "ลำดับ: "; }
		td:nth-of-type(3):before { content: "เลือก: "; }
		td:nth-of-type(4):before { content: "ชื่อรายงาน: "; }
		td:nth-of-type(5):before { content: "URL: "; }
		td:nth-of-type(6):before { content: "กลุ่มรายงาน: "; }
		td:nth-of-type(7):before { content: "ผู้แก้ไข: "; }
		td:nth-of-type(8):before { content: "วันที่แก้ไข: "; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ตั้งค่า รายงาน Power BI</h3>

                    <div class="pull-right">

                        @can('edit-'.str_slug('configs-report-power-bi'))
                        ...
                        @can('add-'.str_slug('configs-report-power-bi'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/config/report-power-bi/create') }}">
                                <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                            </a>
                        @endcan

                        @can('delete-'.str_slug('configs-report-power-bi'))
                    ...
                    {!! Form::model($filter, ['url' => '/config/report-power-bi', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                        ...
                        {!! Form::open(['url' => '/config/report-power-bi/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                        {!! Form::close() !!}

                        {!! Form::open(['url' => '/config/report-power-bi/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="col-md-2">@sortablelink('title', 'ชื่อรายงาน')</th>
                                    <th class="col-md-2">@sortablelink('url', 'URL')</th>
                                    <th>@sortablelink('group_id', 'กลุ่มรายงาน')</th>
                            ...
                            @foreach($ssourl as $item)
                                <tr>
                                    ...
                                    <td class="font-12 text-top" style="word-wrap: anywhere;">{{ $item->url }}</td>
                                    <td class="text-top">{!! !is_null($item->group) ? $item->group->title : '<i class="text-muted">ไม่มีกลุ่ม</i>' !!}</td>
									<td class="text-top">{{ $item->updatedName }}</td>
                                    <td class="text-top">{{ HP::DateThai($item->updated_at) }}</td>
                                    <td class="text-top">
                                      ...
                                      @can('edit-'.str_slug('SsoUrl'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/config/report-power-bi/update-state'],
                                    ...
                                    <td class="text-top">
                                        @can('view-'.str_slug('SsoUrl'))
                                            <a href="{{ url('/config/report-power-bi/' . $item->id) }}"
                                        ...
                                        @can('edit-'.str_slug('SsoUrl'))
                                            <a href="{{ url('/config/report-power-bi/' . $item->id . '/edit') }}"
                                        ...
                                        @can('delete-'.str_slug('SsoUrl'))
                                            {!! Form::open([
                                                            'method'=>'DELETE',
                                                            'url' => ['/config/report-power-bi', $item->id],
                                                            'style' => 'display:inline'
                                            ]) !!}
            ...
            @if(Request::get('sort')=='ordering')
                ...
                $( "#sortable" ).sortable({
                    ...
                    stop:function(event, ui) {
                        ...
                        $.post('{{ url('config/report-power-bi/update_order') }}',
                               {
                                   _token: "{{ csrf_token() }}",
                                   ids: ids,
                                   orders: orders,

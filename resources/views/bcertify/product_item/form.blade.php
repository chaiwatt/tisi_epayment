...
<div class="form-group {{ $errors->has('title') ? 'has-error' : ''}}">
  {!! Form::label('title', 'ชื่อผลิตภัณฑ์ (TH):', ['class' => 'col-md-4 control-label']) !!}
  <div class="col-md-6">
...
<div class="form-group {{ $errors->has('title_en') ? 'has-error' : ''}}">
  {!! Form::label('title_en', 'ชื่อผลิตภัณฑ์ (EN):', ['class' => 'col-md-4 control-label']) !!}
  <div class="col-md-6">
...
<div class="form-group {{ $errors->has('product_category_id') ? 'has-error' : ''}}">
  {!! Form::label('product_category_id', 'หมวดหมู่ผลิตภัณฑ์:', ['class' => 'col-md-4 control-label']) !!}
  <div class="col-md-6">
    {!! Form::select('product_category_id', $product_categorys, null, ['class' => 'form-control', 'required' => 'required', 'placeholder'=>'-เลือกหมวดหมู่ผลิตภัณฑ์-']) !!}
...
<div class="form-group">
  <div class="col-md-offset-4 col-md-4">
    ...
    <a class="btn btn-default" href="{{url('/bcertify/product_item')}}">
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function($) {
    ...
    //เมื่อเลือกสาขาการทดสอบ
    $('#test_branch_id').change(function () {

      $('#product_category_id').children(":not([value=''])").remove();

      var url = '{{ url('bcertify/product_category/list') }}/'+$(this).val();
      $.ajax({
        'type': 'GET',
        'url': url,
        'success': function (datas) {
            ...
            $.each(datas, function(index, data) {
              $('#product_category_id').append('<option value="'+index+'">'+data+'</option>');
            });

...
@section('content')
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <h3 class="box-title pull-left">พารามิเตอร์2 #{{$calibrationBranchInstrumentGroup->name}}</h3>
                ...
                <div class="pull-right">
                 ...
                 @can('edit-'.str_slug('bcertify-scope-lab-cal'))
                  ...
                  @can('add-'.str_slug('bcertify-scope-lab-cal'))
                      <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-two/create/' . $calibrationBranchInstrumentGroup->id) }}">
                ...
                {!! Form::model($filter, ['url' => '/bcertify/setting_scope_lab_cal/instrument-group/parameter-two/'.$calibrationBranchInstrumentGroup->id, 'method' => 'get', 'id' => 'myFilter']) !!}
                <div class="row">
                ...
                <div class="table-responsive">
                  ...
                  {!! Form::open(['url' => '/bcertify/setting_scope_lab_cal/instrument-group/parameter-two/update-state/'.$calibrationBranchInstrumentGroup->id, 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                  ...
                  {!! Form::close() !!}
                    ...
                    <table class="table table-borderless" id="myTable">
                        ...
                        @foreach($calibrationBranchParam2s as $calibrationBranchParam2)
                            <tr>
                                <td>{{ $loop->iteration or $calibrationBranchParam2->id }}</td>
                                <td><input type="checkbox" name="item-selection[]" class="item-selection" value="{{ $calibrationBranchParam2->id }}"></td>
                                <td>{{ $calibrationBranchParam2->name }}</td>
                                <td>
                                  @if($calibrationBranchParam2->state=='1')
                              ...
                              @endif
                                ...
                                <td class="text-right">
                                    ...
                                    @can('view-'.str_slug('bcertify-scope-lab-cal'))
                                        ...
                                        <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-two/show/' . $calibrationBranchParam2->id) }}"
                                           title="View" class="btn btn-info btn-xs">
                                              <i class="fa fa-eye" aria-hidden="true"></i>
                                        </a>
                                    @endcan
                                    @can('edit-'.str_slug('bcertify-scope-lab-cal'))
                                        <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-two/edit/' . $calibrationBranchParam2->id ) }}"
                    ...
                    <div class="pagination-wrapper">
                      {!!
                          $calibrationBranchParam2s->appends([

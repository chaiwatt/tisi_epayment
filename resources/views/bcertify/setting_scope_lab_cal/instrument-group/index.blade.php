...
@section('content')
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <h3 class="box-title pull-left">เครื่องมือ #{{$calibrationBranch->title}}</h3>
                ...
                <div class="pull-right">
                  ...
                  @can('add-'.str_slug('bcertify-scope-lab-cal'))
                      <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/create/' . $calibrationBranch->id) }}">
                  ...
                  <a class="btn btn-info btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_scope_lab_cal') }}" >
                  ...
                  {!! Form::model($filter, ['url' => '/bcertify/setting_scope_lab_cal/instrument-group/'.$calibrationBranch->id, 'method' => 'get', 'id' => 'myFilter']) !!}
                  <div class="row">
                ...
                <div class="table-responsive">
                  ...
                  {!! Form::open(['url' => '/bcertify/setting_scope_lab_cal/instrument-group/update-state/'.$calibrationBranch->id, 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                  ...
                  {!! Form::close() !!}
                    ...
                    <table class="table table-borderless" id="myTable">
                        ...
                        <tbody>
                        @foreach($calibrationBranchInstrumentGroups as $calibrationBranchInstrumentGroup)
                            <tr>
                                <td>{{ $loop->iteration or $calibrationBranchInstrumentGroup->id }}</td>
                                <td><input type="checkbox" name="item-selection[]" class="item-selection" value="{{ $calibrationBranchInstrumentGroup->id }}"></td>
                                <td>{{ $calibrationBranchInstrumentGroup->name }}</td>
                                <td>
                                  @if($calibrationBranchInstrumentGroup->state=='1')
                                    {!! Form::hidden('state', 0) !!}
                                ...
                                <td class="text-right">
                                    @can('view-'.str_slug('bcertify-scope-lab-cal'))
                                        <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/show/' . $calibrationBranchInstrumentGroup->id) }}"
                                           title="View" class="btn btn-info btn-xs">
                                              <i class="fa fa-eye" aria-hidden="true"></i>
                                        </a>
                                    @endcan
                                    @can('edit-'.str_slug('bcertify-scope-lab-cal'))
                                        <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/edit/' . $calibrationBranchInstrumentGroup->id) }}"
                                           title="Edit" class="btn btn-primary btn-xs">
                                              <i class="fa fa-pencil-square-o" aria-hidden="true"> </i>
                                        </a>
                                    @endcan
                                    @can('edit-'.str_slug('bcertify-scope-lab-cal'))
                                      <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/instrument/' . $calibrationBranchInstrumentGroup->id) }}"
                                        title="เครื่องมือ" class="btn btn-success btn-xs">
                                            <i class="fa fa-link" aria-hidden="true"> </i>
                                      </a>
                                      <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-one/' . $calibrationBranchInstrumentGroup->id) }}"
                                        title="พารามิเตอร์1" class="btn btn-warning btn-xs">
                                          <i class="fa fa-link" aria-hidden="true"> </i>
                                      </a>
                                      <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-two/' . $calibrationBranchInstrumentGroup->id) }}"
                                        title="พารามิเตอร์2" class="btn btn-primary btn-xs">
                                          <i class="fa fa-link" aria-hidden="true"> </i>
                                    </a>
                                  @endcan
                                </td>
                    ...
                    <div class="pagination-wrapper">
                      {!!
                          $calibrationBranchInstrumentGroups->appends([

...
@section('content')
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <h3 class="box-title pull-left">พารามิเตอร์1 #{{$calibrationBranchInstrumentGroup->name}}</h3>
                ...
                <div class="pull-right">
                 ...
                 @can('edit-'.str_slug('bcertify-scope-lab-cal'))
                  ...
                  @can('add-'.str_slug('bcertify-scope-lab-cal'))
                      <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-one/create/' . $calibrationBranchInstrumentGroup->id) }}">
                ...
                {!! Form::model($filter, ['url' => '/bcertify/setting_scope_lab_cal/instrument-group/parameter-one/'.$calibrationBranchInstrumentGroup->id, 'method' => 'get', 'id' => 'myFilter']) !!}
                <div class="row">
                ...
                <div class="table-responsive">
                  ...
                  {!! Form::open(['url' => '/bcertify/setting_scope_lab_cal/instrument-group/parameter-one/update-state/'.$calibrationBranchInstrumentGroup->id, 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                  ...
                  {!! Form::close() !!}
                    ...
                    <table class="table table-borderless" id="myTable">
                        ...
                        @foreach($calibrationBranchParam1s as $calibrationBranchParam1)
                            <tr>
                                <td>{{ $loop->iteration or $calibrationBranchParam1->id }}</td>
                                <td><input type="checkbox" name="item-selection[]" class="item-selection" value="{{ $calibrationBranchParam1->id }}"></td>
                                <td>{{ $calibrationBranchParam1->name }}</td>
                                <td>
                                  @if($calibrationBranchParam1->state=='1')
                              ...
                              @endif
                                ...
                                <td class="text-right">
                                    ...
                                    @can('view-'.str_slug('bcertify-scope-lab-cal'))
                                        ...
                                        <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-one/show/' . $calibrationBranchParam1->id) }}"
                                           title="View" class="btn btn-info btn-xs">
                                              <i class="fa fa-eye" aria-hidden="true"></i>
                                        </a>
                                    @endcan
                                    @can('edit-'.str_slug('bcertify-scope-lab-cal'))
                                        <a href="{{ url('/bcertify/setting_scope_lab_cal/instrument-group/parameter-one/edit/' . $calibrationBranchParam1->id ) }}"
                    ...
                    <div class="pagination-wrapper">
                      {!!
                          $calibrationBranchParam1s->appends([

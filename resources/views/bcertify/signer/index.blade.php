...
<style>
  ...
  /*
	...
	*/
    ...
    td:nth-of-type(4):before { content: "ตำแหน่ง:"; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ผู้ลงนาม</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('signer'))
                      ...
                      @can('add-'.str_slug('signer'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/signer/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('signer'))
                    ...
                    {!! Form::model($filter, ['url' => '/bcertify/signer', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/bcertify/signer/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/bcertify/signer/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th>@sortablelink('created_by', 'ชื่อ-สกุล ผู้ลงนาม')</th>
                                <th>@sortablelink('created_by', 'ตำแหน่ง')</th>
                                <th class="col-md-2">ไฟล์ลายเซ็น</th>
                            ...
                            @foreach($signer as $item)
                                <tr>
                                    ...
                                    <td class="text-top">
                                      @if ($item->signature_img!='' && HP::checkFileStorage($attach_path.$item->signature_img))
                                          <a href="{{ HP::getFileStorage($attach_path.$item->signature_img) }}" target="_blank">
                                            <img class="img-responsive img-thumbnail" src="{{ HP::getFileStorage($attach_path.$item->signature_img) }}" width="90%" />
                                          </a>
                                      @endif
                                    </td>
                                    ...
                                    <td class="text-top">
                                      ...
                                      @can('edit-'.str_slug('signer'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/bcertify/signer/update-state'],
                                    ...
                                    <td class="text-top">
                                        @can('view-'.str_slug('signer'))
                                            <a href="{{ url('/bcertify/signer/' . $item->id) }}"
                                               title="View signer" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('signer'))
                                            <a href="{{ url('/bcertify/signer/' . $item->id . '/edit') }}"
                                               title="Edit signer" class="btn btn-primary btn-xs">
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $signer->appends(['search' => Request::get('search'),

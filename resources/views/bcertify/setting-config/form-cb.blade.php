...
@php
           $setting_cb = $setting_config->where('grop_type','cb')->first();
@endphp
...
{!! Form::open(['url' => '/bcertify/setting-config/store',    'method' => 'POST', 'class' => 'form-horizontal', 'files' => true]) !!}
    ...
    <div class="col-md-12">
            ...
            <div class="form-group  required{{ $errors->has('from_filed') ? 'has-error' : ''}}">
                ...
                <div class="col-md-4">
                    {!! Form::select('from_filed',App\Models\Bcertify\SettingConfig::list_from_filed(),!empty($setting_cb->from_filed)?$setting_cb->from_filed:null, ['class' => 'form-control', 'placeholder'=>'- เลือกติดตามครั้งแรกจากวันที่ -', 'required' => true, 'id' => 'from_filed']) !!}
            ...
            <div class="form-group required{{ $errors->has('warning_day') ? 'has-error' : ''}}">
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        {!! Form::text('warning_day', !empty($setting_cb->warning_day)?$setting_cb->warning_day:null, ['class' => 'form-control text-center numberonly', 'required' => 'required']) !!}
            ...
            <div class="form-group required{{ $errors->has('condition_check') ? 'has-error' : ''}}">
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        {!! Form::text('condition_check', !empty($setting_cb->condition_check)?$setting_cb->condition_check:null, ['class' => 'form-control text-center numberonly', 'required' => 'required']) !!}
            ...
            <div class="form-group {{ $errors->has('check_first') ? 'has-error' : ''}}">
                ...
                <div class="col-md-7">
                    <input type="checkbox" class="check item_checkbox" id="check_first" value="1"  name="check_first" data-checkbox="icheckbox_square-green" @if(!empty($setting_cb->check_first) && $setting_cb->check_first==1) checked @endif>
                    <label for="check_first">ตรวจติดตามครั้งแรก 6 เดือน</label>
                </div>
            </div>
       
        </div>
        <input name="grop_type" type="hidden" value="cb">

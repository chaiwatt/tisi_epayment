...
<style>
  ...
  /*
	...
	*/
    ...
    td:nth-of-type(3):before { content: "ชื่อเอกสารแนบ:"; }
    td:nth-of-type(4):before { content: "แบบฟอร์ม:"; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ตั้งค่าเอกสารแนบ</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('config_attach'))
                      ...
                      @can('add-'.str_slug('config_attach'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/config_attach/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('config_attach'))
                    ...
                    {!! Form::model($filter, ['url' => '/bcertify/config_attach', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/bcertify/config_attach/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/bcertify/config_attach/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th>@sortablelink('title', 'ชื่อเอกสารแนบ')</th>
                                <th>แบบฟอร์ม</th>
                            ...
                            <tbody>
                            @foreach($config_attach as $item)
                                <tr>
                                    <td>{{ $loop->iteration or $item->id }}</td>
                                    <td><input type="checkbox" name="cb[]" class="cb" value="{{ $item->id }}"></td>
                                    <td>{{ $item->title }}</td>
                                    <td>@foreach ($item->form_list as $key => $form)@if($key!=0), @endif{{ HP::Forms()[$form->form] }}@endforeach</td>
                                    ...
                                    <td>
                                      ...
                                      @can('edit-'.str_slug('config_attach'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/bcertify/config_attach/update-state'],
                                    ...
                                    <td>
                                        @can('view-'.str_slug('config_attach'))
                                            <a href="{{ url('/bcertify/config_attach/' . $item->id) }}"
                                               title="View config_attach" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('config_attach'))
                                            <a href="{{ url('/bcertify/config_attach/' . $item->id . '/edit') }}"
                                               title="Edit config_attach" class="btn btn-primary btn-xs">
                                        ...
                                        @can('delete-'.str_slug('config_attach'))
                                            {!! Form::open([
                                                            'method'=>'DELETE',
                                                            'url' => ['/bcertify/config_attach', $item->id],
                                                            'style' => 'display:inline'
                                            ]) !!}
                                            {!! Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i>', array(
                                                    'type' => 'submit',
                                                    'class' => 'btn btn-danger btn-xs',
                                                    'title' => 'Delete config_attach',
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $config_attach->appends(['search' => Request::get('search'),

        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <h3 class="box-title pull-left">ตั้งค่าเลขรันระบบใบรับรองระบบงาน</h3>

                    <div class="pull-right">

                        @can('edit-'.str_slug('bcertify_setting_running'))
                        ...
                        @can('add-'.str_slug('bcertify_setting_running'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_running/create') }}">
                                <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                            </a>
                        @endcan

                        @can('delete-'.str_slug('bcertify_setting_running'))
                            ...
                            <table class="table table-striped" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">ระบบงาน (EN)</th>
    ...
    <script>
        ...
        $(function () {
            ...
            var table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/bcertify/setting-running/data_list') !!}',
                    data: function (d) {
            ...
            $(document).on('click', '#bulk_delete', function(){
                ...
                if(id.length > 0){
                    ...
                    if(confirm("ยืนยันการลบข้อมูล " + id.length + " แถว นี้ ?")){
                        $.ajax({
                            type:"POST",
                            url:  "{{ url('/bcertify/setting-running/delete') }}",
                            data:{
                                _token: "{{ csrf_token() }}",
                                id: id
                            },
            ...
            $('#publish, #no_publish').on('click', function () {
                    ...
                    if (arrRowId.length > 0) {
                        ...
                        if (confirm("ยืนยันการ"+text_alert+" " + arrRowId.length + " แถว นี้ ?")) {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ url('/bcertify/setting-running/update_publish') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "id_publish": arrRowId,
                                    "state": state

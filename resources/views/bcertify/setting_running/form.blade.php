...
<div class="form-group required {{ $errors->has('system') ? 'has-error' : ''}}">
    {!! Form::label('system', 'ระบบงาน', ['class' => 'col-md-2 control-label']) !!}
    <div class="col-md-6">
        {!! Form::text('system', null, ('required' == 'required') ? ['class' => 'form-control', 'required' => 'required'] : ['class' => 'form-control']) !!}
        {!! $errors->first('system', '<p class="help-block">:message</p>') !!}
    </div>
</div>
<div class="form-group required {{ $errors->has('system_en') ? 'has-error' : ''}}">
    {!! Form::label('system_en', 'ระบบงาน (EN)', ['class' => 'col-md-2 control-label']) !!}
    <div class="col-md-6">
        {!! Form::text('system_en', null, ('required' == 'required') ? ['class' => 'form-control', 'required' => 'required'] : ['class' => 'form-control']) !!}
        {!! $errors->first('system_en', '<p class="help-block">:message</p>') !!}
    </div>
</div>
...
@if( isset($result->id) )
    ...
    <div class="form-group ">
        ...
        <div class="col-md-6">
            <button class="btn btn-info" type="button" data-toggle="modal" data-target="#Mhistrory">ดูประวัติเปลี่ยนแปลงเลขรัน</button>
        </div>
    </div>

    @include ('bcertify.setting_running.modal')
@endif
...
<fieldset class="white-box">
    ...
    <div class="form-group form-fotmat">
        <div class="col-md-12 box_format_html" data-repeater-list="form-fotmat">
            ...
            @if( isset($result->id) && count( App\Models\Bcertify\SettingRunningSub::where('format_id', $result->id)->get() ) > 0 )

                @php
                    $sub_data = App\Models\Bcertify\SettingRunningSub::where('format_id', $result->id)->get();
                @endphp
...
<div class="form-group">
    <div class="col-md-offset-4 col-md-4">
        ...
        @can('view-'.str_slug('configs-format-code'))
            <a class="btn btn-default show_tag_a" href="{{url('/bcertify/setting_running')}}">

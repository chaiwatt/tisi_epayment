...
@section('content')
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <h3 class="box-title pull-left">พารามิเตอร์ #{{$testBranchCategory->name}}</h3>
                ...
                <div class="pull-right">
                 ...
                 @can('edit-'.str_slug('bcertify-scope-lab-test'))
                  ...
                  @can('add-'.str_slug('bcertify-scope-lab-test'))
                      <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_scope_lab_test/category/parameter/create/' . $testBranchCategory->id) }}">
                  ...
                  <a class="btn btn-info btn-sm waves-effect waves-light" href="{{ url('/bcertify/setting_scope_lab_test/category/'.$testBranchCategory->testBranch->id) }}" >
                ...
                {!! Form::model($filter, ['url' => '/bcertify/setting_scope_lab_test/category/parameter/'.$testBranchCategory->id, 'method' => 'get', 'id' => 'myFilter']) !!}
                <div class="row">
                ...
                <div class="table-responsive">
                  ...
                  {!! Form::open(['url' => '/bcertify/setting_scope_lab_test/category/parameter/update-state/'.$testBranchCategory->id, 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                  ...
                  {!! Form::close() !!}
                    ...
                    <table class="table table-borderless" id="myTable">
                        ...
                        <tr>
                            ...
                            <th style="width:30%">พารามิเตอร์</th>
                        ...
                        @foreach($testBranchParams as $testBranchParam)
                            <tr>
                                <td>{{ $loop->iteration or $testBranchParam->id }}</td>
                                <td><input type="checkbox" name="item-selection[]" class="item-selection" value="{{ $testBranchParam->id }}"></td>
                                <td>{{ $testBranchParam->name }}</td>
                                <td>{{ $testBranchParam->name_eng }}</td>
                                <td>
                                  @if($testBranchParam->state=='1')
                              ...
                              @endif
                                ...
                                <td class="text-right">
                                    ...
                                    @can('view-'.str_slug('bcertify-scope-lab-test'))
                                        ...
                                        <a href="{{ url('/bcertify/setting_scope_lab_test/category/parameter/show/' . $testBranchParam->id) }}"
                                           title="View" class="btn btn-info btn-xs">
                                              <i class="fa fa-eye" aria-hidden="true"></i>
                                        </a>
                                    @endcan
                                    @can('edit-'.str_slug('bcertify-scope-lab-test'))
                                        <a href="{{ url('/bcertify/setting_scope_lab_test/category/parameter/edit/' . $testBranchParam->id ) }}"

...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ประเภทมาตรฐานการรับรอง</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('standardtypes'))
                      ...
                      @can('add-'.str_slug('standardtypes'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bcertify/standardtypes/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('standardtypes'))
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <tr>
                                ...
                                <th  width="15%" class="text-center">ประเภทข้อเสนอ</th>
                                <th width="15%" class="text-center">ประเภทข้อเสนอ (Eng)</th>
                                <th width="15%" class="text-center">กลุ่มงานที่เกี่ยวข้อง</th>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/bcertify/standardtypes/data_list') !!}',
                    data: function (d) {
                ...
                columns: [
                    ...
                    { data: 'offertype', name: 'offertype' },
                    { data: 'offertype_eng', name: 'offertype_eng' },
            ...
            $( "#filter_clear" ).click(function() {
              ...
              table.draw();
            // window.location.assign("{{url('/bcertify/standardtypes')}}");
           });
        ...
        if ($(this).prop('checked')) {//เลือกทั้งหมด
            ...
            $('#myTable tbody').on('click', '.js-state', function(){
                ...
                $.ajax({
                    method: "POST",
                        url: "{{ url('/bcertify/standardtypes/update_status') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id": id,
                            "state": state
            ...
            $('#publish, #no_publish').on('click', function () {
                    ...
                    if (arrRowId.length > 0) {
                        ...
                        if (confirm("ยืนยันการ"+text_alert+" " + arrRowId.length + " แถว นี้ ?")) {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ url('/bcertify/standardtypes/update_publish') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "id_publish": arrRowId,
                                    "state": state
            ...
            $(document).on('click', '#bulk_delete', function(){
                ...
                if(id.length > 0){
                    ...
                    if (confirm("ยืนยันการลบข้อมูล " + id.length + " แถว นี้ ?")) {
                        $.ajax({
                                type:"POST",
                                url:  "{{ url('/bcertify/standardtypes/delete') }}",
                                data:{
                                    _token: "{{ csrf_token() }}",
                                    id: id
                                },

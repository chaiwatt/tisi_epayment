...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ICS</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('ics'))
                      ...
                      @can('add-'.str_slug('ics'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/basic/ics/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('ics'))
                    ...
                    {!! Form::model($filter, ['url' => '/basic/ics', 'method' => 'get', 'id' => 'myFilter']) !!}
                      ...
                      <div class="col-md-5">
												  ...
												  <div class="col-md-9">
														{!! Form::text('filter_search', null, ['class' => 'form-control', 'placeholder'=>'ค้นจาก รหัส (code), ชื่อ, ชื่ออังกฤษ', 'onchange'=>'this.form.submit()']); !!}
												  </div>
											</div>
                    ...
                    <div class="clearfix"></div>
                    <span class="small">{{ 'ทั้งหมด '. $ics->total() .' รายการ'}}</span>
                    <div class="table-responsive">

                      {!! Form::open(['url' => '/basic/ics/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/basic/ics/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th>@sortablelink('title', 'ชื่อ ICS')</th>
                                <th>@sortablelink('title_en', 'ชื่อ ICS (Eng)')</th>
                            ...
                            @foreach($ics as $item)
                                <tr>
                                    <td class="text-top">{{ $ics->perPage()*($ics->currentPage()-1)+$loop->iteration }}</td>
                                    ...
                                    <td>
                                        @can('view-'.str_slug('ics'))
                                            <a href="{{ url('/basic/ics/' . $item->id) }}"
                                               title="View ics" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('ics'))
                                            <a href="{{ url('/basic/ics/' . $item->id . '/edit') }}"
                                               title="Edit ics" class="btn btn-primary btn-xs">
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $ics->appends(['search' => Request::get('search'),

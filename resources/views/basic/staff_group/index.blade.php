	...
	*/
    ...
    You could also use a data-* attribute and content for this. That way "bloats" the HTML, this way means you need to keep HTML and CSS in sync. Lea Verou has a clever way to handle with text-shadow.
		*/
		...
		td:nth-of-type(3):before { content: "ชื่อกลุ่ม"; }
		td:nth-of-type(4):before { content: "กลุ่มผลิตภัณฑ์/สาขา"; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">กลุ่มเจ้าหน้าที่</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('staff_group'))
                      ...
                      @can('add-'.str_slug('staff_group'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/basic/staff_group/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('staff_group'))
                    ...
                    {!! Form::model($filter, ['url' => '/basic/staff_group', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/basic/staff_group/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/basic/staff_group/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th>@sortablelink('order', 'ชื่อกลุ่ม')</th>
                            ...
                            <tbody>
                            @foreach($staff_group as $item)
                                <tr>
                                    <td>{{ $loop->iteration or $item->id }}</td>
                                    <td><input type="checkbox" name="cb[]" class="cb" value="{{ $item->id }}"></td>
                                    <td>{{ $item->order }} : {{ $item->title }}</td>
                                    <td>@foreach ($item->product_group_list as $key => $product_groups)@if($key!=0), @endif{{ $product_groups->product_group->title }}@endforeach</td>
                                    ...
                                    <td>
                                      ...
                                      @can('edit-'.str_slug('staff_group'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/basic/staff_group/update-state'],
                                    ...
                                    <td>
                                        @can('view-'.str_slug('staff_group'))
                                            <a href="{{ url('/basic/staff_group/' . $item->id) }}"
                                               title="View staff_group" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('staff_group'))
                                            <a href="{{ url('/basic/staff_group/' . $item->id . '/edit') }}"
                                               title="Edit staff_group" class="btn btn-primary btn-xs">
                                        ...
                                        @can('delete-'.str_slug('staff_group'))
                                            {!! Form::open([
                                                            'method'=>'DELETE',
                                                            'url' => ['/basic/staff_group', $item->id],
                                                            'style' => 'display:inline'
                                            ]) !!}
                                            {!! Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i>', array(
                                                    'type' => 'submit',
                                                    'class' => 'btn btn-danger btn-xs',
                                                    'title' => 'Delete staff_group',
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $staff_group->appends(['search' => Request::get('search'),

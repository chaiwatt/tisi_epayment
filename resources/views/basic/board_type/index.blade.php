...
<style>
  ...
  /*
	...
	*/
    ...
    td:nth-of-type(3):before { content: "ชื่อประเภท"; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ประเภทของคณะกรรมการ</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('board_type'))
                      ...
                      @can('add-'.str_slug('board_type'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/basic/board_type/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('board_type'))
                    ...
                    {!! Form::model($filter, ['url' => '/basic/board_type', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/basic/board_type/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/basic/board_type/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th class="text-center" width="35%">@sortablelink('title', 'ชื่อประเภทของคณะกรรมการ')</th>
                                <th class="text-center" width="20%">@sortablelink('expert_group_id', 'กลุ่มประเภทคณะกรรมการ')</th>
                                <th class="text-center" width="13%">@sortablelink('created_by', 'ผู้สร้าง')</th>
																<th class="text-center" width="10%">@sortablelink('created_at', 'วันที่สร้าง')</th>
                                <th class="text-center" width="10%">@sortablelink('state', 'สถานะ')</th>
                                <th class="text-center" width="10%">จัดการ</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($board_type as $item)
                                <tr>
                                    <td>{{ $loop->iteration or $item->id }}</td>
                                    <td><input type="checkbox" name="cb[]" class="cb" value="{{ $item->id }}"></td>
                                    <td>{{ $item->title }}</td>
                                    <td>{{  !empty($item->expert_group_to->title) ?  $item->expert_group_to->title : null }}</td>
                                    ...
                                    <td>
                                      ...
                                      @can('edit-'.str_slug('board_type'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/basic/board_type/update-state'],
                                    ...
                                    <td>
                                        @can('view-'.str_slug('board_type'))
                                            <a href="{{ url('/basic/board_type/' . $item->id) }}"
                                               title="View board_type" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('board_type'))
                                            <a href="{{ url('/basic/board_type/' . $item->id . '/edit') }}"
                                               title="Edit board_type" class="btn btn-primary btn-xs">
                                        ...
                                        @can('delete-'.str_slug('board_type'))
                                            {!! Form::open([
                                                            'method'=>'DELETE',
                                                            'url' => ['/basic/board_type', $item->id],
                                                            'style' => 'display:inline'
                                            ]) !!}
                                            {!! Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i>', array(
                                                    'type' => 'submit',
                                                    'class' => 'btn btn-danger btn-xs',
                                                    'title' => 'Delete board_type',
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $board_type->appends(['search' => Request::get('search'),

...
@section('content')
            ...
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <div class="pull-right">
                        @can('add-'.str_slug('law-listen-ministry-track'))
                            <button type="button"  class="btn btn-success waves-effect waves-light"  id="ButtonModal">
                                <span class="btn-label"><i class="fa fa-plus"></i></span>ประกาศราชกิจจานุเบกษา
                            </button>
                        @endcan
                    </div>
                            ...
                            <table class="table table-striped" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" width="25%">ชื่อเรื่องประกาศ</th>
                                        <th class="text-center" width="10%">มาตรฐาน (มอก.)</th>
    ...
    @include ('laws.listen.ministry-track.modal')
@endsection
        ...
        $(document).ready(function () {
            ...
            $("body").on("click", "#ButtonModal", function() {
                ...
                if(ids.length > 0){
                    ...
                    Swal.fire({
                                position: 'center',
                                icon: 'warning',
                                title: 'กรุณาเลือกชื่อเรื่องประกาศ',
           ...
           //ปฎิทิน
            ...
            table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/law/listen/ministry-track/data_list') !!}',
                    data: function (d) {
            ...
            $(document).on('change', '.js-switch', function(e) {
                ...
                if (confirm("ยืนยันการ"+text_alert+ "ข้อมูลแถว นี้ ?")) {
                    ...
                    $.ajax({
                        method: "put",
                        url: "{{ url('law/listen/ministry-track/update-state') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_publish": ids,
                            "state": state

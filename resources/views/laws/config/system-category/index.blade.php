            ...
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <div class="pull-right">
                        @can('add-'.str_slug('law-config-system-category'))
                            <a class="btn btn-success waves-effect waves-light" href="{{ url('/law/config/system-category/create') }}">
                        ...
                        <div class="col-md-12">
                            <div class="pull-right has-dropdown">
                                ...
                                <ul role="menu" class="dropdown-menu">
                                    @can('edit-'.str_slug('law-config-system-category'))
                                        <li>
                                    ...
                                    @endcan
                                    @can('delete-'.str_slug('law-config-system-category'))
                                        <li>
                            ...
                            <table class="table table-striped" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" width="23%">ชื่อเรื่อง</th>
                                        <th class="text-center" width="10%">สีที่แสดง</th>
                                        <th class="text-center" width="20%">ผู้บันทึก</th>
                                        <th class="text-center" width="15%">ระบบแจ้งเตือน</th>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/law/config/system-category/data_list') !!}',
                    data: function (d) {
                ...
                columnDefs: [
                    { className: "text-center text-top", targets:[1,-1,-2,-3,-4,-5] },
            ...
            $(document).on('click', '.btn_update_state', function(e) {
                ...
                if (confirm("ยืนยันการ"+text_alert+ "ข้อมูลแถว นี้ ?")) {
                    ...
                    $.ajax({
                        method: "put",
                        url: "{{ url('law/config/system-category/update-state') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_publish": ids,
                            "state": state
            ...
            $(document).on('click', '.btn_update_notify', function(e) {
                ...
                if (confirm("ยืนยันการ"+text_alert+ "ข้อมูลแถว นี้ ?")) {
                    ...
                    $.ajax({
                        method: "put",
                        url: "{{ url('law/config/system-category/update-notify') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_publish": ids,
                            "state": state
        ...
        function Delete(){
            ...
            if (ids.length > 0) {
                ...
                if(confirm("ยืนยันการลบข้อมูล " + ids.length + " แถว นี้ ?")){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "{{ url('law/config/system-category/delete') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_publish": ids,
                        }

        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <h3 class="box-title pull-left">ตั้งค่าแจ้งเตือน</h3>

                    <div class="pull-right">
                        @can('add-'.str_slug('law-config-notification'))
                            <a class="btn btn-success waves-effect waves-light" href="{{ url('/law/config/notification/create') }}">
                        ...
                        <div class="col-md-12">
                            <div class="pull-right has-dropdown">
                                ...
                                <ul role="menu" class="dropdown-menu">
                                    @can('edit-'.str_slug('law-config-notification'))
                                        <li>
                                    ...
                                    @endcan
                                    @can('delete-'.str_slug('law-config-notification'))
                                        <li>
                            ...
                            <table class="table table-striped" id="myTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" width="25%">ชื่อเรื่อง</th>
                                        {{-- <th class="text-center" width="15%">สีที่แสดง</th> --}}
                                        <th class="text-center" width="15%">เงื่อนไข</th>
                                        {{-- <th class="text-center" width="10%">จำนวน(วัน)</th> --}}
                                        <th class="text-center" width="13%">ผู้บันทึก</th>
                                        <th class="text-center" width="13%">ผู้แก้ไข</th>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/law/config/notification/data_list') !!}',
                    data: function (d) {
            ...
            $(document).on('click', '.btn_update_state', function(e) {
                ...
                if (confirm("ยืนยันการ"+text_alert+ "ข้อมูลแถว นี้ ?")) {
                    ...
                    $.ajax({
                        method: "put",
                        url: "{{ url('law/config/notification/update-state') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_publish": ids,
                            "state": state
        ...
        function Delete(){
            ...
            if (ids.length > 0) {
                ...
                if(confirm("ยืนยันการลบข้อมูล " + ids.length + " แถว นี้ ?")){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "{{ url('law/config/notification/delete') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_publish": ids,
                        }

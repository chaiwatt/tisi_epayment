...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ข้อมูลผู้ลงนาม (Signer)</h3>

                    <div class="pull-right">

                      @can('edit-'.str_slug('signers'))
                      ...
                      @can('add-'.str_slug('signers'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/besurv/signers/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('signers'))
                    ...
                    {!! Form::model($filter, ['url' => '/besurv/signers', 'method' => 'get', 'id' => 'myFilter']) !!}
                      ...
                      <div class="col-md-4">
												  {!! Form::label('title', 'กลุ่มงานหลัก:', ['class' => 'col-md-4 control-label label-filter']) !!}
												  <div class="col-md-8">
                            {!! Form::select('filter_main_group', App\Models\Besurv\Department::selectRaw('CONCAT(depart_nameShort," ",depart_name) As title, did')->pluck('title', 'did'), null, ['class' => 'form-control', 'id'=>'filter_main_group', 'placeholder'=>'-เลือก กลุ่มงานหลัก-', 'onchange'=>'this.form.submit()']); !!}
												  </div>
											</div>
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/besurv/signers/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/besurv/signers/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                                ...
                                <tr>
                                    ...
                                    <td>
                                      ...
                                      @can('edit-'.str_slug('signers'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/besurv/signers/update-state'],
                                    ...
                                    <td>
                                        @can('view-'.str_slug('signers'))
                                            <a href="{{ url('/besurv/signers/' . $item->id) }}"
                                        ...
                                        @can('edit-'.str_slug('signers'))
                                            <a href="{{ url('/besurv/signers/' . $item->id . '/edit') }}"
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $signers->appends(['search' => Request::get('search'),
                                                      'sort' => Request::get('sort'),
                                                      'direction' => Request::get('direction'),
                                                      'perPage' => Request::get('perPage'),
                                                      'filter_state' => Request::get('filter_state'),
                                                      'filter_main_group' => Request::get('filter_main_group'),
                                                     ])->render()
                          !!}
                        </div>
                    </div>

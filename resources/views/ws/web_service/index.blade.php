...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ผู้ใช้งานเว็บเซอร์วิส API</h3>

                    <div class="pull-right">

                        @can('edit-'.str_slug('web_service'))

                            <button class="btn btn-info btn-sm btn-outline waves-effect waves-light btn_modal_mail" type="button" >
                                <span class="btn-label"><i class="fa fa-paper-plane"></i></span><b>ส่งเมล</b>
                            </button>
                        ...
                        @can('add-'.str_slug('web_service'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/ws/web_service/create') }}">
                                <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                            </a>
                        @endcan

                        @can('delete-'.str_slug('web_service'))
                    ...
                    {!! Form::model($filter, ['url' => '/ws/web_service', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/ws/web_service/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/ws/web_service/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>@sortablelink('title', 'ชื่อผู้ใช้งาน Web Service')</th>
                                    <th>@sortablelink('app_name', 'app-name')</th>
                                    <th>@sortablelink('email', 'อีเมลติดต่อ')</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @can('edit-'.str_slug('web_service'))
                                          ...
                                          {!! Form::open([
                                                'method'=>'PUT',
                                                'url' => ['/ws/web_service/update-state'],
                                    ...
                                    <td>
                                        @can('view-'.str_slug('web_service'))
                                            <a href="{{ url('/ws/web_service/' . $item->id) }}"
                                               title="View web_service" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('web_service'))
                                            <a href="{{ url('/ws/web_service/' . $item->id . '/edit') }}"
                                               title="Edit web_service" class="btn btn-primary btn-xs">
                                        ...
                                        @can('delete-'.str_slug('web_service'))
                                            {!! Form::open([
                                                            'method'=>'DELETE',
                                                            'url' => ['/ws/web_service', $item->id],
                                                            'style' => 'display:inline'
                                            ]) !!}
                                            {!! Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i>', array(
                                                    'type' => 'submit',
                                                    'class' => 'btn btn-danger btn-xs',
                                                    'title' => 'Delete web_service',
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $web_service->appends(['search' => Request::get('search'),
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.btn_modal_mail').on('click', function () {

                if( $('#myTable').find('input.cb:checked').length > 0 ){//ถ้าเลือกส่ง
                    var arrRowId = [];

                    //Iterate over all checkboxes in the table
                    $('#myTable').find('input.cb:checked').each(function (index, rowId) {
                        arrRowId.push(rowId.value);
                    });


                    if (confirm("ยืนยันการการส่งเมล" + arrRowId.length + " แถว นี้ ?")) {

                        Swal.fire({
                            title: 'กำลังส่งอีเมลโปรดรอ.....',
                            type: 'info',
                        ...
                        $.ajax({
                            method: "POST",
                            url: "{{ url('/ws/web_service/send-mail') }}",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "id_publish": arrRowId,
                            },
                            success : function (object){
                                ...
                                if (object.msg == "success") {
                                    ...
                                    $.each(object.error, function( index, data ) {
                                        html = '<p>'+data+'</p>';
                                    });
                    ...
                    alert("กรุณาเลือกข้อมูลที่ต้องส่งเมล");
                }

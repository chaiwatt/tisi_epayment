...
{!! Form::open(['url' => 'certify/certificate-export-lab/add_attach/'.$export_lab->id, 'class' => 'form-horizontal', 'files' => true,'id'=>'form_add_attach']) !!}
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group {{ $errors->has('app_no') ? 'has-error' : ''}}">
                                ...
                                <div class="col-md-6">
                                    {!! Form::text('app_no', $export_lab->request_number, ['class' => 'form-control','id'=>'app_no', 'readonly' => true]) !!}
                ...
                <div id="table_evidence">
                    ...
                    @if( isset($file_all) && count($file_all) > 0)
                        <div class="row">
                            <div class="col-sm-12">
                                                ...
                                                <tbody>
                                                    @foreach($certilab_file_all as $key => $certilab_file)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <p class="text-left">
                                                                    @if(!is_null($certilab_file->attach))
                                                                        <a href="{!! HP::getFileStorage($certilab_file->attach) !!}" target="_blank">
                                                                            {!! HP::FileExtension($certilab_file->attach) ?? '' !!}
                                                                        </a>
                                                                    @endif
                                                                    @if(!is_null($certilab_file->attach_pdf))
                                                                        <a href="{!! HP::getFileStorage($certilab_file->attach_pdf) !!}" target="_blank">
                                                            ...
                                                            <td>
                                                                <div class="checkbox"><input class="js-switch " name="state" type="checkbox" value="{{$certilab_file->id}}" {{ ($certilab_file->state == 1) ? 'checked ' : '' }}></div>
                                                            </td>
                                                            <td>
                                                                {{ HP::dateTimeFormatN($certilab_file->created_at) ?? '-' }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
            ...
            <div class="modal-footer">
                <input type="hidden" name="app_certi_lab_id" value="{{ $export_lab->id }}">

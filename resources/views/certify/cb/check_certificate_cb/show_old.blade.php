        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <hr>
                    <a class="btn {{ ($certi_cb->status >= 6) ? 'btn-info' : 'btn-warning'  }} "   href="{{ url('certify/check_certificate-cb/show/'.$certi_cb->app_no) }}" >
...
@if(auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("29",auth()->user()->RoleListId))         
                     @if($certi_cb->status >= 6 && !is_null($certi_cb->CertiCBCostTo))
                                ...
                                @endphp
                            <a  class="btn {{$cost_btn}}" href="{{  url('certify/estimated_cost-cb/'.$Cost->id.'/edit') }}" target="_blank">
 ...
 @endphp
   <div class="btn-group">
      <div class="btn-group">
             <a  class="btn  {{$auditors_btn}} " href="{{ url("certify/auditor-cb")}}" target="_blank">
    ...
    @else 
        <a  class="btn btn-warning" href="{{ url("certify/auditor-cb")}}" target="_blank">
            แต่งตั้งคณะฯ
        </a>
    @endif
@endif 
...
@endphp
    <div class="btn-group">
        <div class="btn-group">
                <a  class="btn {{$assessment_btn}}" href="{{ url("certify/save_assessment-cb")}}" target="_blank">
...
@endforeach
    ...
    @else 
         <a  class="btn btn-warning" href="{{ url("certify/save_assessment-cb")}}" target="_blank">
            ผลการตรวจประเมิน
        </a>
    @endif
...
@if(auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("26",auth()->user()->RoleListId))   
                @if($certi_cb->status >= 14)
                  ...
                  <!-- Button แนบใบ Pay-in ครั้งที่ 2  -->
                   <a  class="btn {{$payin2_btn}} " href="{{ url("certify/check_certificate-cb/Pay_In2/".$payin2->id."/".$certi_cb->token)}}">
                   ...
                   @endif
                      ...
                      @if($certi_cb->status >= 17)
                        ...
                        @include ('certify/cb/check_certificate_cb/modal.modalstatus22',['file_all'=> $certi_cb->CertiCBFileAlls,
                                                                                         'certi_cb' => $certi_cb 
                                                                                        ])

                          @if(!is_null($certi_cb->CertiCBExportTo))
                                    <a   class="btn btn-info" href="{{ url('certify/certificate-export-cb/'.$certi_cb->CertiCBExportTo->id.'/edit') }}">
                                        <i class="fa fa-check-square-o"></i>  ออกใบรับรอง
                                    </a> 
                            @else 
                            <a  class="btn btn-warning" href="{{  url('certify/certificate-export-cb') }}" target="_blank">
                               ออกใบรับรอง
                            </a>
                            @endif 
                   @endif
                                          ...
                                          <div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
                                              ...
                                              <div class="col-md-7">
                                                @if($certi_cb->status < 6)
                                                    {!! Form::select('status',
                                                    App\Models\Certify\ApplicantCB\CertiCBStatus::whereNotIn('id',[0])->whereIN('id',[1,2,3,4,5,6])->pluck('title', 'id'), 
                                                    $certi_cb->status ?? null, 
                                                    ['class' => 'form-control', 

    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group {{ $errors->has('modal_app_no') ? 'has-error' : ''}}">
                                ...
                                <div class="col-md-6">
                                    {!! Form::text('modal_app_no', !empty($certi_cb_primary->app_no)?$certi_cb_primary->app_no :null, ['class' => 'form-control','id'=>'modal_app_no', 'readonly' => true]) !!}
            ...
            <div class="modal-footer" style="text-align: center">
                ...
                <button type="button" id="save_evidence" class="btn btn-primary">บันทึก</button>
            </div>
        </div>
    </div>
</div>
            ...
            $('#save_evidence').click(function (e) { 
                ...
                if(  !checkNone(start_date)  ||  !checkNone(end_date) ){
                    ...
                    var formData = new FormData();
                        ...
                        formData.append('app_no',$('#modal_app_no').val());
                      //  formData.append('app_certi_cb_id','{{ !empty($export->app_certi_cb_id)?  $export->app_certi_cb_id:'' }}');
                        formData.append('app_certi_cb_id','{{ !empty($certi_cb_primary->id)?  $certi_cb_primary->id:'' }}');
                        formData.append('ref_id','{{ $certi_cb->id }}');
                        formData.append('form','create');
                    $.ajax({
                        type: "POST",
                        url: "{{ url('/certify/certificate_detail-cb/update_document') }}",
                        datatype: "script",
                        data: formData,
                        contentType: false,
                        cache: false,
                        ...
                        success: function (msg) {
                            if (msg != "") {
                                ...
                                var tr_ = '';
                                    ...
                                    tr_ += '<td class="text-center">'+(msg.app_no)+'</td>'; 

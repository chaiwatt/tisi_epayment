...
{{-- 
                      @can('edit-'.str_slug('saveassessmentcb'))
                      ...
                      @can('add-'.str_slug('saveassessmentcb'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/certify/save_assessment-cb/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      {{-- @can('delete-'.str_slug('saveassessmentcb'))
                    ...
                    <hr>
                    {!! Form::model($filter, ['url' => '/certify/save_assessment-cb', 'method' => 'get', 'id' => 'myFilter']) !!}
                    <div class="row">
                ...
                <input type="hidden" name="direction" value="{{ Request::get('direction') }}" />
                    ...
                    <div class="clearfix"></div>
                             ...
                             @foreach($assessment as $item)
                                ...
                                <tr>
                                    ...
                                    <td  class="text-center">
                                      @if(in_array($item->degree,['0','1','4']))
                                           @can('edit-'.str_slug('saveassessmentcb'))
                                              <a href="{{ url('certify/save_assessment-cb/'.$item->id.'/edit') }}" class="btn btn-light btn-primary">
                                      ...
                                      @else 
                                        ...
                                        @can('edit-'.str_slug('saveassessmentcb'))
                                                <a href="{{ url('certify/save_assessment-cb/assessment/'.$item->id.'/edit') }}" class="btn btn-light btn-info">
                                                  <i class="fa fa-pencil-square-o"></i>
                                              </a>
                                         @endcan
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $assessment->appends(['search' => Request::get('search'),
    ...
    <script>
        $(document).ready(function () {
            ...
            $( "#filter_clear" ).click(function() {
                window.location.assign("{{ url('/certify/save_assessment-cb') }}");
            });

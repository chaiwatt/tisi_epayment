...
<div id="collapse" class="panel-collapse collapse in">
    <br>
  @foreach($auditorcb->CertiCbHistorys as $key => $item)
        ...
        <div class="col-md-1"></div>
            ...
            <div class="container-fluid">
               ...
               <div class="row">
                 ...
                 <div class="col-md-7">
                     {{-- @if($item->attachs !='' && HP::checkFileStorage($attach_path.$item->attachs)) --}}
                     <a href="{{url('certify/check/file_cb_client/'.$item->attachs.'/'.( !empty($item->attach_client_name) ? $item->attach_client_name : basename($item->attachs) ))}}" target="_blank">
                ...
                @foreach($attachs_file as $files)
                        <p> 
                            ...
                            {{-- @if($files->file !='' && HP::checkFileStorage($attach_path.$files->file)) --}}
                             <a href="{{url('certify/check/file_cb_client/'.$files->file.'/'.( !empty($files->file_client_name) ? $files->file_client_name : basename($files->file) ))}}" target="_blank">

                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group {{ $errors->has('modal_app_no') ? 'has-error' : ''}}">
                                ...
                                <div class="col-md-6">
                                    {!! Form::text('modal_app_no', !empty($certi_ib_primary->app_no)?  $certi_ib_primary->app_no :null, ['class' => 'form-control','id'=>'modal_app_no', 'readonly' => true]) !!}
            ...
            $('#save_evidence').click(function (e) { 
                ...
                if(  !checkNone(start_date)  ||  !checkNone(end_date) ){
                    ...
                    var formData = new FormData();
                       ...
                       // formData.append('app_certi_ib_id','{{ !empty($export->app_certi_ib_id)?  $export->app_certi_ib_id:'' }}');
                        formData.append('app_certi_ib_id','{{ !empty($certi_ib_primary->id)?  $certi_ib_primary->id:'' }}');
                        formData.append('ref_id','{{ $certi_ib->id }}');
                        formData.append('form','create');
                    $.ajax({
                        type: "POST",
                        url: "{{ url('/certify/certificate_detail-ib/update_document') }}",
                        datatype: "script",
                        data: formData,
                        contentType: false,
                        cache: false,

...
@section('content')
   ...
   @if(is_null($payin2->degree) || (!is_null($payin2) && $payin2->degree == 1))
        {!! Form::open(['url' => 'certify/check_certificate-ib/create/pay-in2/'.@$payin2->id, 'class' => 'form-horizontal pay_in2_form', 'files' => true,'id'=>'pay_in2_form']) !!}
    @else
        {!! Form::open(['url' => 'certify/check_certificate-ib/update/pay-in2/'.@$payin2->id, 'class' => 'form-horizontal pay_in2_form', 'files' => true,'id'=>'pay_in2_form']) !!}
    @endif
 ...
 <div class="form-group {{ $errors->has('list') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6 m-t-10">
      <label for=""> {!! $payin2->CertiIBCostTo->app_no ?? null  !!} </label>
    </div>
</div>
    ...
    <div class="col-md-6 m-t-10">
        <div class="table-responsive">
            <table class="table color-bordered-table info-bordered-table table-bordered" >
                ...
                <tbody>
                    @if (!is_null($payin2->CertiIBCostTo))
                        ...
                        @php
                            ...
                            $purpose_type  = $payin2->CertiIBCostTo->standard_change;
                        @endphp
...
<div class="form-group div-collect {{ $errors->has('other_attach') ? 'has-error' : ''}}" id="div-attach">
    ...
    <div class="col-md-7">
        <a href="{{url('certify/check/file_ib_client/'.$payin2->FileAttachPayInTwo1To->file.'/'.( !empty($payin2->FileAttachPayInTwo1To->file_client_name) ? $payin2->FileAttachPayInTwo1To->file_client_name :  basename($payin2->FileAttachPayInTwo1To->file) ))}}" target="_blank">
...
<div class="form-group {{ $errors->has('list') ? 'has-error' : ''}}">
    ...
    <div class="col-md-3 m-t-9">
        <a href="{{url('certify/check/file_ib_client/'.$payin2->FileAttachPayInTwo2To->file.'/'.( !empty($payin2->FileAttachPayInTwo2To->file_client_name) ? $payin2->FileAttachPayInTwo2To->file_client_name :   basename($payin2->FileAttachPayInTwo2To->file) ))}}" target="_blank">
        ...
        $("input[name=conditional_type]").on("ifChanged",function(){
         ...
         $('#save_pay_in').click(function () {
            ...
            if(row == '1'){ // เรียกเก็บค่าธรรมเนียม
                        ...
                        $.ajax({
                            type:"post",
                            url:  "{{ url('/certify/check_certificate_ib/check_pay_in_ib') }}",
                            data:{
                                _token: "{{ csrf_token() }}",
                                id:  "{{ $payin2->id ?? null }}",
                                payin : '2'
         ...
         $('#transaction_payin').click(function () {
                ...
                var ref1          =   "{{ $payin2->CertiIBCostTo->app_no ?? null }}";

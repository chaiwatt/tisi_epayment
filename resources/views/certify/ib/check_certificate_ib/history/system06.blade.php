...
<div class="col-md-8 text-left">
 @php
     $auditors =  App\Models\Certify\ApplicantIB\CertiIBAuditors::where('id',$payin1->auditors_id)->first();
 @endphp
 {{ !empty($auditors->CertiIBAuditorsDateTitle) ? $auditors->CertiIBAuditorsDateTitle  : null }}
</div>
</div>
@endif
...
<div class="col-md-8 text-left">
    <p> 
        @if(isset($history->attachs_file))
        <a href="{{url('certify/check/file_ib_client/'.$history->attachs_file.'/'.( !empty($history->evidence) ? $history->evidence : basename($history->attachs_file) ))}}" target="_blank">

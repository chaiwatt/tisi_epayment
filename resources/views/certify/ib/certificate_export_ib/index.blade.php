            ...
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <div class="pull-right">
                        {{-- @can('add-'.str_slug('certificateexportib'))
                            <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/certify/certificate-export-ib/create') }}">
                    ...
                    {!! Form::model($filter, ['url' => '/certify/certificate-export-ib', 'method' => 'get', 'id' => 'myFilter']) !!}
                        <div class="row">
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tbody>
                                ...
                                @if( count($export_ib) == 0)
                                    <tr><td class="text-center" colspan="8">ไม่พบข้อมูล</td></tr>  
                                @endif
                                @foreach($export_ib as $item)
                                    <tr>
                                        <td class="text-center">{{ $loop->iteration + ( ((request()->query('page') ?? 1) - 1) * $export_ib->perPage() ) }}</td>
                                        <td>
                                            {{  $item->app_no ?? '-'  }}
                                        </td>
                                        <td>
                                    
                                            {{  $item->CertiIBCostTo->name ?? '-'  }}
                                        </td>
                                        <td>
                                        {{  $item->certificate ?? '-'  }}
                                        </td>
                                        ...
                                        <td class="text-center" >
                                            <a href="{{ url('certify/certificate-export-ib/'.$item->id.'/edit') }}" class="btn  ">
    ...
    <script>
        $(document).ready(function () {
          $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/certify/certificate-export-ib')}}");
            });

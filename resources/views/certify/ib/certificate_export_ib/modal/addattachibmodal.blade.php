...
{!! Form::open(['url' => 'certify/certificate-export-ib/add_attach/'.$export_ib->id, 'class' => 'form-horizontal', 'files' => true,'id'=>'form_add_attach']) !!}
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group {{ $errors->has('app_no') ? 'has-error' : ''}}">
                                ...
                                <div class="col-md-6">
                                    {!! Form::text('app_no', $export_ib->app_no, ['class' => 'form-control','id'=>'app_no','required' => true,'readonly' => true]) !!}
                                    ...
                                    <div class="col-md-8 text-left ">
                                        <div class="table-responsive">
                                            <table class="table color-bordered-table info-bordered-table">
                                                ...
                                                <tbody>
                                                    @foreach($certiib_file_all as $key => $certiib_file)
                                                    <tr>
                                                        <td>{{ $key+1 }}.</td>
                                                        <td>
                                                            <p class="text-left">
                                                                @if(!is_null($certiib_file->attach))
                                                                    <a href="{!! HP::getFileStorage($certiib_file->attach) !!}" target="_blank">
                                                                        {!! HP::FileExtension($certiib_file->attach) ?? '' !!}
                                                                    </a>
                                                                @endif
                                                                @if(!is_null($certiib_file->attach_pdf))
                                                                    <a href="{!! HP::getFileStorage($certiib_file->attach_pdf) !!}" target="_blank">
                                                        ...
                                                        <td>
                                                            <div class="checkbox"><input class="js-switch " name="state" type="checkbox" value="{{$certiib_file->id}}" {{ ($certiib_file->state == 1) ? 'checked ' : '' }}></div>
                                                        </td>
                                                        <td>
                                                            {{ HP::DateThai($certiib_file->created_at) ?? '-' }}
                                                        </td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
            ...
            <div class="modal-footer">
                <input type="hidden" name="app_certi_ib_id" value="{{ $export_ib->id }}">

...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ออกใบรับรองงาน</h3>
{{--                    @can('view-'.str_slug('board'))--}}
                    ...
                    {!! Form::open(['url' => 'certify/certificate-export/create/'.$lang,'method'=>'POST','id'=>'addForm', 'class' => 'form-horizontal', 'files' => true]) !!}
                    ...
                    <div class="tab-content">
                            ...
                            <div class="white-box">
                                ...
                                <div class="form-group">
                                    @php $name = 'certificate_for' ;
                                         ...
                                         $option_text = ($lang == "th" ? '- เลือกออกใบรับรองฉบับนี้ให้ -': '- Select issue this certificate for -') ;
                                    @endphp
                                    <label for="requestNumber" class="col-md-4 control-label"><span class="text-danger">*</span> {{$text}} :</label>
                                    <div class="col-md-6">
                                        <select name="{{$name}}" id="{{$name}}" class="form-control" required>
                                                 <option value=""  selected>{{$option_text}}</option>
                                            @foreach($requests as $request)
                                                <p>$request</p>
                                                <option value="{{$request->id}}">{{$request->trader_name}}</option>
                                            @endforeach
                                        </select>
                                ...
                                <input type="hidden" name="lab_type" id=lab_type" class="lab_type form-control">
                                ...
                                <div class="form-group">
                                    @php $name = 'certificate_date_start' ; $text = ($lang == "th" ? 'ออกให้ ณ วันที่': 'Issue date') @endphp
                                    <label for="requestNumber" class="col-md-4 control-label"><span class="text-danger">*</span> {{$text}} :</label>
                                    <div class="col-md-2">
                                        <input type="text" class="form-control datepicker" placeholder="{{$text}}" name="{{$name}}" id="{{$name}}" autocomplete="off" required>
                                        {!! $errors->first($name, '<p class="help-block">:message</p>') !!}
                                    </div>
                                    @php $name = 'certificate_date_end' ; $text = ($lang == "th" ? 'สิ้นสุดวันที่': 'Valid date') @endphp
                                ...
                                <div class="form-group">
                                    @php $name = 'certificate_date_first' ; $text = ($lang == "th" ? 'ออกให้ครั้งแรก ณ วันที่': 'Date of initial issue') @endphp
                                    <label for="requestNumber" class="col-md-4 control-label"><span class="text-danger">*</span> {{$text}} :</label>
                                    <div class="col-md-2">
                                        <input type="text" class="form-control datepicker" placeholder="{{$text}}" name="{{$name}}" id="{{$name}}" autocomplete="off" required>
                                        {!! $errors->first($name, '<p class="help-block">:message</p>') !!}
                        ...
                        <div id="other-th" class="tab-pane fade">
                            @include('certify.certificate-export.layout.other')
                        </div>
                    </div>

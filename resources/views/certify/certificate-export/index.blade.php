...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ระบบออกใบรับรอง</h3>

                    <div class="pull-right">

                      @can('add-'.str_slug('certificate'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ route('certificate-export.create',['lang'=>'th']) }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ route('certificate-export.create',['lang'=>'en']) }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>Create</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('certificate'))
                    ...
                    {!! Form::model($filter, ['url' => 'certify/certificate-export', 'method' => 'get', 'id' => 'myFilter']) !!}
...
{{--                        </div>--}}
                    ...
                    <div class="col-md-1 m-t-15 {{isset($_GET['perPage']) == true ? 'show':'hide'}}">
                        <label>&emsp;</label>
                        <a href="{{url('certify/certificate-export')}}" class="btn btn-primary btn-block">คืนค่า</a>
                    </div>
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th class="text-center" width="100px">ใบรับรอง</th>
                            </tr>
                            </thead>
                            <tbody>
                            @if ($certificates and $certificates->count() > 0)
                                @foreach ($certificates as $certificate)
                                    <tr>
                                        ...
                                        <td class="text-center">{{$certificate->lab_type_name ?? '-'}}</td>
                                        <td class="text-center"><b>{{$certificate->certificate_no ?? '-'}}</b></td>
                                        <td class="text-center">
                                            {{$certificate->status_text ?? '-'}}
                                        </td>
                                        <td class="text-center">{{\Carbon\Carbon::parse($certificate->certificate_date_start)->format("d/m/Y")}}</td>
                                        <td class="text-center">
                                            <a href="{{route('certificate-export.edit',['cer'=>$certificate])}}">
                                                @if ($certificate->lang == "th")
                                                <img src="{{ asset('storage/uploads/certify/thailand.png') }}" height="30px"/>
                                                @else
                                                    <img src="{{ asset('storage/uploads/certify/united-kingdom.png') }}" height="30px"/>

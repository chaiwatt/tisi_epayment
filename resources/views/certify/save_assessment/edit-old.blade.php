        ...
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    ...
                    {!! Form::open(['url' => route('save_assessment.update', ['notice'=> $notice, 'app' => @$notice->applicant->id ? : '']), 'class' => 'form-horizontal', 'method' => 'put']) !!}
...
{{--                                        <div v-for="label in appLabels">--}}
{{--                                            <input type="text" class="form-control" v-model="label.text" name="department" disabled>--}}
{{--                                            <br>--}}
{{--                                        </div>--}}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" class="form-control mydatepicker" name="savedate" value="{{ !empty($notice->assessment_date) ?  HP::revertDate($notice->assessment_date->format('Y-m-d'),true)  : null }}" autocomplete="off" required>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-md-6">
                                    ...
                                    <div class="col-md-7">
                                        <div class="row">
                                            <label class="col-md-6"><input type="radio" name="optradio" value="1" {{ $notice->report_status==1?'checked':'' }} @click="isoptradio()" required> มี</label>
                                            <label class="col-md-6"><input type="radio" name="optradio" value="2" {{ $notice->report_status==2?'checked':'' }} @click="isNotoptradio()" required> ไม่มี</label>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        new Vue({
            ...
            data: {
                ...
                isDraft: true,
                appNo: '{{ $notice->applicant->app_no }}',
                appDepart: '{{ $notice->applicant->trader->trader_operater_name }}',
                appLabName: '{{ $notice->applicant->lab_name }}',
                appOptions: [],
                labels: [],
                appGroupId: "",
                isTable: false
            ...
            mounted() {
                ...
                @foreach($group->auditors as $ag)
                    ...
                    @if ($notice->group != null && in_array($ag->id, $notice->group))
                        this.labels.push({text: '{{ $ag->auditor->no }}', value: '{{ $ag->id }}'})
                    @endif
                @endforeach

                @foreach($notice->applicant->assessment->getSelectAuditors() as $af)
                    this.founds.push({text: '{{ $af->fname_th.' '.$af->lname_th }}', value: '{{ $af->id }}'});
                @endforeach
                this.assessments = [];
                ...
                console.log("Save Assessment Edit Mounted.");
            }
        });

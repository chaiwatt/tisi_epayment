...
<div class="col-md-12 repeater">
    ...
    <table class="table color-bordered-table primary-bordered-table">
        ...
        @forelse($ba->groups as $key => $group)

            <tr class="repeater-item">
                <td class="text-center text-top">
                    {{-- {{$group->status_auditor_id}} --}}
                    <div class="form-group {{ $errors->has('taxid') ? 'has-error' : ''}}">
                        <div class="col-md-9">
                            {!! Form::select('status', $status_auditor,
                              $group->status_auditor_id, ['class' => 'form-control item status', 'placeholder'=>'-เลือกสถานะผู้ตรวจประเมิน-', 'data-name'=>'status', 'required'=>true, 'data-status' => $key]); !!}
                        </div>
                    </div>
                </td>
                {{-- จะแสดงข้อมูลชื่อผู้ทบทวนฯ จากการติ๊กเลือกใน popup  --}}
                ...
                <td>
                    {!! Form::hidden('category', null, ['class' => 'form-control item', 'placeholder'=>'','data-name'=>'category','required' => true, 'readonly', 'data-status' => $key]); !!}
                </td>
                <td align="center" class="text-top">
                    <button type="button" class="btn btn-danger btn-xs repeater-remove">
                        <i class="fa fa-trash" aria-hidden="true"></i>
  ...
  <script src="https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            // วนลูปไปที่แต่ละ select ใน table
            $('select[name^="status"]').each(function() {
                var selectedText = $(this).find("option:selected").text(); // หาค่าที่เลือกจาก select
                var statusData = $(this).data('status'); // ดึง data-status ของ select

                // หา input ที่มี data-name ตรงกับ statusData
                var inputField = $('input[data-name="category"][data-status="' + statusData + '"]');
                
                // ตั้งค่า value ของ input ให้เป็นค่าที่เลือกจาก select

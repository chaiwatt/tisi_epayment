        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    {!! Form::model($filter, ['url' => '/tis/listen_std_draft', 'method' => 'get', 'id' => 'myFilter']) !!}
                        <div class="row">
                            <div class="col-lg-3">
                    ...
                    <div class="clearfix"></div>
                    <span class="small">{{ 'ทั้งหมด '. $listen_std_draft->total() .' รายการ'}}</span>
                    <div class="table-responsive">

                      {!! Form::open(['url' => '/tis/listen_std_draft/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/tis/listen_std_draft/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tbody>
                            @foreach($listen_std_draft as $item)
                                <tr>
                                    {{-- <td>{{ $loop->iteration or $item->id }}</td> --}}
                                    <td class="text-top">{{ $listen_std_draft->perPage()*($listen_std_draft->currentPage()-1)+$loop->iteration }}</td>
                                    <td><input type="checkbox" name="cb[]" class="cb" value="{{ $item->id }}"></td>
                                    <td>{{ HP::DateThai($item->created_at) }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->departmentNameName }}</td>
                                    <td>{{ $item->note_std_draft->title ?? "n/a" }}</td>
                                    <td>{{ $item->note_std_draft->tis_no ?? "n/a"}}</td>
									                  <td>{{ !is_null($item->note_std_draft) ? $item->note_std_draft->ProductGroupName : "n/a" }}</td>
                                    <td>{{ $item->tel }}</td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        {{-- @can('edit-'.str_slug('listen_std_draft'))
                                            <a href="{{ url('/tis/listen_std_draft/' . $item->id . '/edit') }}"
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $listen_std_draft->appends(['sort' => Request::get('sort'),
    ...
    <script>
        $(document).ready(function () {
            ...
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/tis/listen_std_draft')}}");
            });

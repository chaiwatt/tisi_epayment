            ...
            <div class="col-sm-12">
                <div class="white-box">
                            ...
                            <hr class="hr-line bg-primary">
                                ...
                                {!! Form::model($filter, ['url' => '/asurv/accept21_import', 'method' => 'get', 'id' => 'myFilter']) !!}
                 ...
                 <div class="clearfix"></div>
                    ...
                    <div class="table-responsive">
                            <table class="table table-striped" id="myTable">
                                ...
                                @foreach($accept21_import as $list)
                                    <tr>
                                        <td>{{$temp_num++}}.</td>
                                        <td>{{$list->ref_no}}</td>
                                        ...
                                        <td>
                                            ...
                                            @php
                                                if($list->state === 1 ||$list->state === 2 ||$list->state === 3){
                                                    $url = url("/asurv/accept21_import/$list->id/edit");
                                                    ...
                                                    $url = url("/asurv/accept21_import/detail/$list->id");
                                            ...
                                            @endif
{{-- 
                                            @if(HP_API_PID::check_api('check_api_asurv_accept21_import') && HP_API_PID::CheckDataApiPid($list, (new App\Models\Asurv\EsurvBiss21)->getTable()) != '' &&  $list->state === 1)
                                                <span data-id="{{$list->id}}" data-url="{!! $url !!}" data-table="{!! (new App\Models\Asurv\EsurvBiss21)->getTable() !!}" class="check_api_pid glyphicon glyphicon-search btn-lg"></span>
                                            @else
                                                @if($check_a == true)
                                                    <a href="{{ $url }}">{!! $icon !!}</a>
                                                @endif
                                        ...
                                        <td>
                                            ...
                                            @elseif($list->state_check === 1)
                                                <a href="{{url("/asurv/accept21_import/update_status/$list->id/$list->state_check")}}"> <i class="fa fa-check-circle fa-2x" style="color: #1ec01e"></i></a>
                                            @elseif($list->state_check === 0)
                                                <a href="{{url("/asurv/accept21_import/update_status/$list->id/$list->state_check")}}"><i class="fa fa-times-circle fa-2x" style="color: red"></i></a>
                                            @endif
                                        </td>
                                        <td>
                            ...
                            <div class="pagination-wrapper">
                                ...
                                @endphp
                                {!!
                                    $accept21_import->appends($page)->links()
                                !!}
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/asurv/accept21_import')}}");
            });

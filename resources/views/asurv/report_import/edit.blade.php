        ...
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    <div class="row">
                        <div class="col-md-12">
                            <h1 class="box-title">ระบบรับแจ้งปริมาณการทำผลิตภัณฑ์เพื่อใช้ในประเทศเป็นการเฉพาะคราว (20 ทวิ)</h1>
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <div class="col-sm-8 ">
                                        <input type="text" class="form-control" disabled
                                               value="{{HP::get_ref_no2_4($data->applicant_20bis_id)}}">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-10">
                                    ...
                                    <div class="col-sm-4">
                                        <input type="text" class="form-control" disabled
                                               value="{{HP::get_title2_4($data->applicant_20bis_id)}}">
                                    </div>
                                    <div class="col-sm-2 text-right">ระยะเวลาที่แจ้งผลิต</div>
                                    <div class="col-sm-4">
                                        <input type="text" class="form-control" disabled
                                               value="{{ HP::DateThai(HP::get_date_start2_4($data->applicant_20bis_id)) }} - {{ HP::DateThai(HP::get_date_end2_4($data->applicant_20bis_id)) }}">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                    ...
                                    @foreach($data_detail as $item)
                                        <tr>
                                            ...
                                            <td><input type="text" class="form-control text-center" disabled
                                                       value="{{HP::get_detail2_4($item->detail_id)}}"></td>
                                            ...
                                            <td> {{HP::get_unit_name(HP::get_id_unit($item->detail_id))}}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                                            ...
                                            <div class="other_attach_item">
                                                ...
                                                <div class="col-md-3">
                                                    <div class="fileinput fileinput-new input-group pull-left col-md-10" data-provides="fileinput">
                                                        <div >
                                                            {{-- <a href="{{url('/asurv/report_import/download/'.$attach->file_name)}}">{{$attach->file_client_name}}</a> --}}
                                                ...
                                                <div class="col-md-2">
                                                    {{-- <a href="{{url('/asurv/report_import/preview/'.$attach->file_name)}}" target="_blank" class="view-attach btn btn-info btn-sm"><i class="fa fa-search"></i></a> --}}
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                        ...
                                        <div class="table-responsive">
                                            <table class="table color-bordered-table primary-bordered-table"
                                                ...
                                                @foreach ($data_volume_detail as $list)
                                                    <input hidden id="{{$list->detail_id}}_{{$list->volume_20bis_id}}" value="{{$list->quantity}}">
                                                    <?php
                                                    $a++;
                                                    ?>
                                                    <input hidden value="{{$b[] = $list->detail_id}}">
                                                    <input hidden value="{{$c[] = $list->volume_20bis_id}}">
                                                @endforeach
                                                <?php
                                                $i = 0;
                                                $k = 0;
                                                ...
                                                @foreach($data_detail as $key => $item)
                                                    <tr>
                                                        ...
                                                        <td><input type="text" class="form-control text-center"
                                                                   disabled value="{{HP::get_detail2_4($item->detail_id)}}"></td>
                                                        <td><input type="text" class="form-control text-center"
                                                                   value="{{HP::get_quantity2_4($item->detail_id)}}" disabled></td>
                                                        @foreach($data_volume_main as $key => $list)
                                                            <td>
                                                                <input type="text"
                                                                       class="form-control text-center"
                                                                       id="table2_Q_{{$item->detail_id}}_{{$data_detail_ck[$key]->volume_20bis_id}}"
                                                                       disabled
                                                                       value="0">
                                                            </td>
                                                        @endforeach
                                                        <td><input type="text" id="table2_{{$row++}}"
                                                                   class="form-control text-center"
                                                                   disabled value="{{HP::get_sum_quantity2_4($data->applicant_20bis_id,$item->detail_id)}}">
                                                        </td>
                                                        <td><label>{{HP::get_unit_name(HP::get_id_unit($item->detail_id))}}</label></td>
                                                    </tr>
...
@push('js')
    ...
    $('.datepicker').datepicker({
        ...
        $('#form_data').on('submit', function (event) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('/asurv/report_import/save')}}",
                datatype: "script",
                data: form_data,
                contentType: false,
                cache: false,
                ...
                success: function (data) {
                    if (data.status == "success") {
                        window.location.href = "{{url('/asurv/report_import')}}"

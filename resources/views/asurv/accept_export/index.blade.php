        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    {!! Form::model($filter, ['url' => '/asurv/accept_export', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                            ...
                            <table class="table table-striped" id="myTable">
                                ...
                                @foreach($accept_export as $list)
                                      ...
                                      <tr>
                                        ...
                                        <td>
                                            @if($list->state === 4)
                                                <a href="{{ url('/asurv/accept_export/pdf_download/' . $list->id) }}"
                                            ...
                                            @php
                                                if($list->state === 1 ||$list->state === 2 ||$list->state === 3){
                                                    $url = url("/asurv/accept_export/$list->id/edit");
                                                    ...
                                                    $url = url("/asurv/accept_export/detail/$list->id");
                                            ...
                                            @if($check_a == true)
                                                <a href="{{ $url }}">{!! $icon !!}</a>
                                            @endif
                                            {{-- @if(HP_API_PID::check_api('check_api_asurv_accept_export') && HP_API_PID::CheckDataApiPid($list, (new App\Models\Asurv\EsurvTers20)->getTable()) != '' && $list->state === 1 )
                                                <span data-id="{{ $list->id }}" data-url="{!! $url !!}" data-table="{!! (new App\Models\Asurv\EsurvTers20)->getTable() !!}" class="check_api_pid glyphicon glyphicon-search btn-lg"></span>
                                            @else
                                        ...
                                        <td>
                                            ...
                                            @elseif($list->state_check === 1)
                                                <a href="{{url("/asurv/accept_export/update_status/$list->id/$list->state_check")}}"> <i class="fa fa-check-circle fa-2x" style="color: #1ec01e"></i></a>
                                            @elseif($list->state_check === 0)
                                                <a href="{{url("/asurv/accept_export/update_status/$list->id/$list->state_check")}}"><i class="fa fa-times-circle fa-2x" style="color: red"></i></a>
                                            @endif
                                        </td>
                                        <td>
                            ...
                            <div class="pagination-wrapper">
                                ...
                                @endphp
                                {!!
                                    $accept_export->appends($page)->links()
                                !!}
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/asurv/accept_export')}}");
            });

                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <hr class="hr-line bg-primary">
                                ...
                                {!! Form::model($filter, ['url' => '/asurv/report_export', 'method' => 'get', 'id' => 'myFilter']) !!}
                 ...
                 <div class="table-responsive">
                            <table class="table table-striped" id="myTable">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 5%;">เลขที่คำขออ้างอิง</th>
                                    <th style="width: 5%;">ผู้ยื่นแจ้งปริมาณ</th>
                                    <th style="width: 8%;">ชื่อผลิตภัณฑ์</th>
                                    <th style="width: 7%;">วันที่ผลิต</th>
                                    <th style="width: 6%;">วันที่แจ้ง</th>
                                ...
                                @php $status_css = ['0'=>'label-success', '1'=>'label-danger'];
                                    ...
                                    // dd($report_export);
                                @endphp
                                @foreach($report_export as $list)
                                    <tr>
                                        <td>{{$temp_num++}}.</td>
                                        <td>{{ !empty($list->esurv_ters20_to->ref_no) ? $list->esurv_ters20_to->ref_no : null }}</td>
                                        <td>{{HP::get_name_4($list->created_by)}}</td>
                                        <td>{{ !empty($list->esurv_ters20_to->title) ? $list->esurv_ters20_to->title : null }}</td>
                                        <td>{{ HP::DateThai($list->start_date) }}
                                            – {{ HP::DateThai($list->end_date) }}</td> 
                                        <td>{{ HP::DateThai($list->created_at) }}</td>
                                        <td>
                                            {{-- @if(in_array('All', $user_tis_list) || in_array(@$list->TisName, $user_tis_list)) --}}
                                            @if(in_array('All', $user_tis_list) || count(array_intersect($list->TisNos, $user_tis_list)) > 0)
                                             @php
                                                    // var_dump($list->TisNos);
                                                    // var_dump($user_tis_list);
                                                @endphp 
                                            <a href="{{url("/asurv/report_export/$list->id/edit")}}"><span
                            ...
                            <div class="pagination-wrapper">
                                ...
                                @endphp
                                {!!
                                    $report_export->appends($page)->links()
                                !!}
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/asurv/report_export')}}");
            });

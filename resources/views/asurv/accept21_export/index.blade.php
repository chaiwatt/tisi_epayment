        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                            ...
                            <h1 class="box-title">ระบบรับคำขอการนำเข้าผลิตภัณฑ์เพื่อส่งออก (21 ตรี)</h1>
                            <hr class="hr-line bg-primary">


                                {!! Form::model($filter, ['url' => '/asurv/accept21_export', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                            ...
                            <table class="table table-striped" id="myTable">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 8%;">แผนการนำเข้า</th>
                                ...
                                @foreach($accept21_export as $list)
                                    <tr>
                                        <td>{{$temp_num++}}.</td>
                                        <td>{{$list->ref_no}}</td>
                                        <td>{{HP::get_name_4($list->created_by)}}</td>
                                        <td>{{$list->title}}</td>
                                        <td>{{ HP::DateThai($list->start_import_date) }} – {{ HP::DateThai($list->end_import_date) }}</td>
                                        <td>{{ HP::DateThai($list->created_at) }}</td>
                                        <td>

                                            @if($list->state === 4)
                                                <a href="{{ url('/asurv/accept21_export/pdf_download/' . $list->id) }}"
                                                        title="ดาวน์โหลดใบรับแจ้ง 20ตรี">
                                            ...
                                            @php
                                                if($list->state === 1 ||$list->state === 2 ||$list->state === 3){
                                                    $url = url("/asurv/accept21_export/$list->id/edit");
                                                    ...
                                                    $url = url("/asurv/accept21_export/detail/$list->id");
                                            ...
                                            {{-- @if(HP_API_PID::check_api('check_api_asurv_accept21_export') && HP_API_PID::CheckDataApiPid($list,(new App\Models\Asurv\EsurvTers21)->getTable())  != '' &&  $list->state === 1)
                                                <span  data-id="{{$list->id}}" data-url="{!!  $url !!}" data-table="{!! (new App\Models\Asurv\EsurvTers21)->getTable() !!}" class="check_api_pid glyphicon glyphicon-search btn-lg">   </span>
                                            @else
                                                @if(  $check_a == true )
                                                    <a href="{{ $url }}">{!! $icon !!}</a>
                                                @endif
                                        ...
                                        <td>
                                            ...
                                            @elseif($list->state_check === 1)
                                                <a href="{{url("/asurv/accept21_export/update_status/$list->id/$list->state_check")}}"> <i class="fa fa-check-circle fa-2x" style="color: #1ec01e"></i></a>
                                            @elseif($list->state_check === 0)
                                                <a href="{{url("/asurv/accept21_export/update_status/$list->id/$list->state_check")}}"><i class="fa fa-times-circle fa-2x" style="color: red"></i></a>
                                            @endif
                                        </td>
                                        <td>
                            ...
                            <div class="pagination-wrapper">
                                ...
                                @endphp
                                {!!
                                    $accept21_export->appends($page)->links()
                                !!}
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/asurv/accept21_export')}}");
            });
            ...
            $('#filter_department').change(function(){
                ...
                if(department_id!=""){
                    $.ajax({
                        type: "GET",
                        url: "{{url('/ssurv/save_example/accept21_export')}}",
                        datatype: "html",
                        data: {

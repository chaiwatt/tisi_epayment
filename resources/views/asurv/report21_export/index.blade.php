                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <hr class="hr-line bg-primary">
                             ...
                             {!! Form::model($filter, ['url' => '/asurv/report21_export', 'method' => 'get', 'id' => 'myFilter']) !!}
                 ...
                 <div class="table-responsive">
                            <table class="table table-striped" id="myTable">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 3%;">เลขที่คำขออ้างอิง</th>
                                    <th style="width: 5%;">ผู้ยื่นแจ้งปริมาณ</th>
                                    <th style="width: 5%;">ชื่อผลิตภัณฑ์</th>
                                    <th style="width: 5%;">แผนการผลิต</th>
                                    <th style="width: 5%;">แผนการนำเข้า</th>
                                    <th style="width: 5%;">แผนการส่งออก</th>
                                    <th style="width: 5%;">วันที่แจ้ง</th>
                                    <th style="width: 4%;">รายละเอียดการ แจ้งปริมาณ</th>
                                    <th style="width: 4%;">การแจ้งปริมาณ</th>
                                ...
                                @endphp
                                @foreach($report21_export as $list)
                                    <tr>
                                        <td>{{$temp_num++}}.</td>
                                        <td>{{HP::get_ref_no1_5($list->applicant_21ter_id)}}</td>
                                        <td>{{HP::get_name_5($list->created_by)}}</td>
                                        <td>{{HP::get_title1_5($list->applicant_21ter_id)}}</td>
                                        <td>{{ HP::DateThai($list->applicant->start_date)??'n/a' }}<br> – <br>{{ HP::DateThai($list->applicant->end_date)??'n/a' }}
                                        </td>
                                        <td>{{ HP::DateThai($list->applicant->start_import_date)??'n/a' }}<br> – <br>{{ HP::DateThai($list->applicant->end_import_date)??'n/a' }}
                                        </td>
                                        <td>{{ HP::DateThai($list->applicant->start_export_date)??'n/a' }}<br> – <br>{{ HP::DateThai($list->applicant->end_export_date)??'n/a' }}
                                        </td>
                                        <td>{{ HP::DateThai($list->created_at) }}</td>
                                        <td>
                                   
                                            @if(in_array('All', $user_tis_list)  || in_array(@$list->TisName, $user_tis_list) )
                                            <a href="{{url("/asurv/report21_export/$list->id/edit")}}"><span
                                        ...
                                        <td>
                                            @if($list->officer_report == null)
                                                -
                                            @else
                                                {{HP::get_create_5($list->officer_report)}}
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <div class="pagination-wrapper">
                                ...
                                @endphp
                                {!!
                                    $report21_export->appends($page)->links()
                                !!}
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/asurv/report21_export')}}");
            });

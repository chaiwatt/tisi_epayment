...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">รับแจ้งปริมาณการผลิตตามเงื่อนไข</h3>
                    ...
                    {!! Form::model($filter, ['url' => '/esurv/receive_volume', 'method' => 'get', 'id' => 'myFilter']) !!}
                    <div class="row">
                            <div class="col-md-4 form-group">
                              <div class="col-md-12">
                                ...
                                {!! Form::select('filter_created_by',
                                                App\Models\Esurv\ReceiveVolume::select(DB::raw("CONCAT($sso_user_table.name) AS title"), "$sso_user_table.id")
                                                    ->Join($sso_user_table, "$sso_user_table.id", '=', 'esurv_inform_volumes.created_by')
                                                    ->distinct('esurv_inform_volumes.created_by')
                                                    ->pluck('title', 'id'),
                                                null,
                ...
                <div id="search-btn" class="panel-collapse collapse">
                      <div class="white-box" style="display: flex; flex-direction: column;">
                          ...
                          <div class="row">
                            <div class="form-group col-md-6">
                            {!! Form::label('filter_tis', 'มาตรฐาน:', ['class' => 'col-md-3 control-label label-filter']) !!}
                            <div class="col-md-8">
                              {!! Form::select('filter_tis', App\Models\Esurv\ReceiveVolume::pluck('tb3_Tisno', 'tb3_Tisno'), null, ['class' => 'form-control', 'placeholder'=>'-เลือกมาตรฐาน-']); !!}
                            </div>
                         </div>
                        </div>
                      </div>
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/esurv/receive_volume/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/esurv/receive_volume/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                          ...
                          <table class="table table-borderless" id="myTable">
                            <thead>
                              <tr>
                                  ...
                                  <th>@sortablelink('inform_year', 'ปี')</th>
  																<th class="col-md-2">@sortablelink('created_at', 'วันที่แจ้ง')</th>
                            ...
                            @foreach($receive_volume as $item)
                                ...
                                <td class="text-top">{{ $receive_volume->perPage()*($receive_volume->currentPage()-1)+$loop->iteration }}</td>
                                    ...
                                    <td class="text-top">
                                      @if($item->state == 1)
                                            @can('edit-'.str_slug('receive_volume'))
                                            @if($user_tis->first()=='All' || in_array(@$item->tis->tb3_Tisno, $user_tis_list) )
                                                <a href="{{ url('/esurv/receive_volume/' . $item->id . '/edit') }}"
                                      ...
                                      @else
                                            ...
                                            @if($user_tis->first()=='All' || in_array(@$item->tis->tb3_Tisno, $user_tis_list) )
                                              <a href="{{ url('/esurv/receive_volume/' . $item->id) }}"
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $receive_volume->appends(['search' => Request::get('search'),
                                                      ...
                                                      'filter_tis' => Request::get('filter_tis'),
                                                     ])->render()
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#filter_tis").select2({minimumInputLength: 2});
            ...
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/esurv/receive_volume')}}");
            });

            if( $('#filter_start_month').val()!="" ||  $('#filter_end_month').val() != ""    ||  $('#filter_department').val() != ""  ||  $('#filter_sub_department').val() != ""  ||  $('#filter_tis').val() != ""){
                // alert('มีค่า');

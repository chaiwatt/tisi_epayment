...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">รับแจ้งผลการประเมิน QC</h3>
                    ...
                    {!! Form::model($filter, ['url' => '/esurv/receive_quality_control', 'method' => 'get', 'id' => 'myFilter']) !!}
                        <div class="row">
                            <div class="col-md-4 form-group">
                                <div class="col-md-12">
                                    ...
                                    {!! Form::select('filter_created_by',
                                            App\Models\Esurv\ReceiveQualityControl::select(DB::raw("CONCAT($sso_user_table.name) AS titels"), "$sso_user_table.id")
                                                              ->Join($sso_user_table, "$sso_user_table.id", '=', 'esurv_inform_quality_controls.created_by')
                                                              ->distinct('esurv_inform_quality_controls.created_by')
                                                              ->pluck('titels','id') ,
                                            null,
                  ...
                  <div id="search-btn" class="panel-collapse collapse">
                    <div class="white-box" style="display: flex; flex-direction: column;">
                        <div class="row">
                          <div class="form-group col-md-6">
                            ...
                            <div class="col-md-8">
                              {!! Form::select('filter_tb3_Tisno', HP::TisList(), null, ['class' => 'form-control', 'placeholder'=>'-เลือกมาตรฐาน-']); !!}
                            </div>
                          </div>
                          <div class="form-group col-md-6">
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/esurv/receive_quality_control/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/esurv/receive_quality_control/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                           ...
                           <table class="table table-borderless" id="myTable">
                            ...
                            <tbody>
                              ...
                              @endphp
                              @foreach($receive_quality_control as $item)
                                <tr>
                                    {{-- <td class="text-top">{{ $loop->iteration or $item->id }}</td> --}}
                                    <td class="text-top">{{ $receive_quality_control->perPage()*($receive_quality_control->currentPage()-1)+$loop->iteration }}</td>
                                ...
                                @if($item->state == 1)
                                      ...
                                      @can('edit-'.str_slug('receive_quality_control'))
                                      @if($user_tis->first()=='All' ||   (!empty($item->tis->tb3_Tisno)  && in_array($item->tis->tb3_Tisno, $user_tis_list)))
                                          <a href="{{ url('/esurv/receive_quality_control/' . $item->id . '/edit') }}"
                                ...
                                @else
                                    ...
                                    @if($user_tis->first()=='All' || (!empty($item->tis->tb3_Tisno)  && in_array($item->tis->tb3_Tisno, $user_tis_list)))
                                        <a href="{{ url('/esurv/receive_quality_control/' . $item->id) }}"
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $receive_quality_control->appends(['search' => Request::get('search'),
    ...
    <script>
        $(document).ready(function () {
            ...
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/esurv/receive_quality_control')}}");
            });

...
<style>
  ...
  /*
	...
	*/
    ...
    td:nth-of-type(5):before { content: "รายละเอียด:"; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">รับแจ้งการเปลี่ยนแปลงที่มีผลต่อคุณภาพ</h3>
                    ...
                    {!! Form::model($filter, ['url' => '/esurv/receive_change', 'method' => 'get', 'id' => 'myFilter']) !!}
                        ...
                        <div class="col-md-4 form-group">
                            <div class="col-md-12">
                                ...
                                {!! Form::select('filter_created_by',
                                    App\Models\Esurv\ReceiveChange::select(DB::raw("CONCAT($sso_user_table.name) AS title"), "$sso_user_table.id")
                                                              ->Join($sso_user_table, "$sso_user_table.id", '=', 'esurv_inform_changes.created_by')
                                                              ->distinct('esurv_inform_changes.created_by')
                                                              ->pluck('title','id') ,
                                 null,
                                 ['class' => 'form-control',
                              ...
                              <div class="col-md-8">
                                <div class="input-daterange input-group" id="date-range">
                                  ...
                                  {{-- <span class="input-group-btn">
                                    <button type="button" class="btn btn-success btn-sm" onclick="this.form.submit()">
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/esurv/receive_change/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/esurv/receive_change/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                              ...
                              <th class="col-md-3">@sortablelink('detail', 'รายละเอียด')</th>
                            ...
                            @endphp
                            @foreach($receive_change as $item)
                                <tr>
                                    {{-- <td class="text-top">{{ $loop->iteration or $item->id }}</td> --}}
                                    <td class="text-top">{{ $receive_change->perPage()*($receive_change->currentPage()-1)+$loop->iteration }}</td>
                                    ...
                                    <td class="text-top">
                                      ...
                                      @else
                                              @can('edit-'.str_slug('receive_change'))
                                              @if($user_tis->first()=='All' || (!empty($item->tis->tb3_Tisno)  && in_array($item->tis->tb3_Tisno, $user_tis_list)))
                                                  <a href="{{ url('/esurv/receive_change/' . $item->id) }}"
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $receive_change->appends(['search' => Request::get('search'),
    ...
    <script>
        $(document).ready(function () {
            ...
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/esurv/receive_change')}}");
            });

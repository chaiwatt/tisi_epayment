            ...
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <div class="pull-right">
                            @can('add-'.str_slug('control_check'))
                                <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/csurv/control_check/create') }}">
                                <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                                </a>
                            @endcan
                            @can('delete-'.str_slug('control_check'))
                    ...
                    <div class="clearfix"></div>
                     <hr>
                     {!! Form::model($filter, ['url' => '/csurv/control_check', 'method' => 'get', 'id' => 'myFilter']) !!}
                     <div class="row">
                ...
                <div class="clearfix"></div>
                            ...
                            <table class="table table-striped" id="myTable">
                                ...
                                <tbody>
                                @foreach($control_check as $key =>  $item)
                                    <tr>
                                        <td class="text-center">{{ $control_check->perPage()*($control_check->currentPage()-1)+$loop->iteration }}.</td>
                                        ...
                                        <td>
                                            @php
                                            $operation =    ['1' => 'ไม่ดำเนินการใดๆ','2' => 'ส่งให้กองกฏหมายดำเนินการ'] ;  
                                            @endphp
                                            @if(array_key_exists($item->operation,$operation))
                                             {{
                                        ...
                                        <td>
                                            @if($item->status_check != '-')
                                                 {{  !empty($item->User_FullName->FullName) ? $item->User_FullName->FullName : '-' }}
                                            @else 
                                              {{$item->status_check}}
                                            @endif
                                        ...
                                        <td class="text-center">
                                            @if($item->status == '0' ||  $item->status == '5')
                                                <a href="{{url("/csurv/control_check/$item->id/edit")}}"
                                                   class="btn btn-primary btn-xs">
                                                   <i class="fa fa-pencil-square-o" aria-hidden="true"> </i>
                                                </a>
                                            @elseif($item->status == '2' || $item->status == '4')
                                                <a href="{{url("/csurv/control_check/detail/$item->id")}}"
                            ...
                            <div class="pagination-wrapper">
                                ...
                                @endphp
                                {!!
                                    $control_check->appends($page)->links()
                                !!}
                            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $( "#filter_clear" ).click(function() {
                ...
                window.location.assign("{{url('/csurv/control_check')}}");
            });

            if( $('#filter_tb3_Tisno').val()!="" ||$('#filter_start_month').val() != "" || $('#filter_end_month').val() != "" ){
                // alert('มีค่า');
        ...
        function confirm_delete(id) {
            if (confirm('ยืนยันการลบข้อมูล?')) {
                $.ajax({
                type: "POST",
                url: "{{url('/csurv/control_check/delete_status')}}",
                datatype: "html",
                data: {
                    id: id,
                    '_token': "{{ csrf_token() }}",
                        ...
                        var rowsSelect = $('.cb:checked').length;
                         if(confirm("คุณแน่ใจหรือว่าต้องการลบข้อมูลนี้ " + rowsSelect + " แถว นี้ ?"))
                         {
                             ...
                             if(id.length > 0)
                             {
                                 $.ajax({
                                     type:"POST",
                                     url:  "{{ url('csurv/control_check/delete') }}",
                                     data:{
                                      "_token": "{{ csrf_token() }}",
                                       id:id},
                                     success:function(data)

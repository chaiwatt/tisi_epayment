...
ขอแก้ไขขอบข่าย สำหรับคำขอเลขที่    {{  !empty($certi_cb->app_no) ?   $certi_cb->app_no  :  ''  }} 
...
@else
    ตามที่ท่านได้ยื่นคำขอรับรองหน่วยรับรอง
สำหรับคำขอเลขที่    {{  !empty($certi_cb->app_no) ?   $certi_cb->app_no  :  ''  }} 
เมื่อวันที่  {{  !empty($certi_cb->created_at) ?  HP::formatDateThaiFull($certi_cb->created_at) :  ''  }}  นั้น
ทางเจ้าหน้าที่จึงขอแจ้งผ่านการตรวจประเมินและขอความเห็นเรื่อง Scope  จาก   {{  !empty($certi_cb->name) ?  $certi_cb->name   :  ''  }}
@endif
         ...
         @if(count($assessment->FileAttachAssessment2Many) > 0)
           <p > 
                   ...
                   <p class="indent100">
                       <a href="{{ url('certify/check/files_cb/'.$item2->file) }}" target="_blank" >
                         {!!  $item2->file   !!}
                     </a>
                   </p>    
                   @endforeach

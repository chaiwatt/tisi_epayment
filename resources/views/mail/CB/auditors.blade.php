...
<body>
   <div id="style">
        ...
        <p class="indent50"> 
            กำหนดการ  {{  !empty($auditors->CertiCBAuditorsDateTitle ) ?  $auditors->CertiCBAuditorsDateTitle   :  ''  }}    
        </p>
        @if(count($auditors->CertiCBAuditorsLists) > 0)
        <table  class="customers">
            ...
            <tbody>
                    ...
                    @foreach($auditors->CertiCBAuditorsLists as $key => $item) 
                     <tr>
                        <td class="center">{{$key+1}}</td>
                        <td>{{ $item->CertiCbAuditorsStatusTo->StatusAuditorTitle ?? '-'}}</td>
        ...
        @if(count($auditors->CertiCBAuditorsCosts) > 0)
        <table  class="customers">
            <thead>
                    <tr>
                        <th width="5%" >ลำดับ</th>
            ...
            <tbody>
                    @php
                        $total = 0;   
                    @endphp
                    @foreach($auditors->CertiCBAuditorsCosts as $key => $item) 
                    @php
                    ...
                    @endphp
                     <tr>
                        <td class="center">{{$key+1}}</td> 
                        <td>{{ @$item->StatusAuditorTo->title  ?? '-' }}</td>

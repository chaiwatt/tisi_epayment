...
<body>
   <div id="style">
    ...
    <p style="text-indent: 50px;">
        ตามที่ท่านได้ยื่นคำขอรับบ่ริการ หมายเลขคำขอ      {{  !empty($certi_ib->app_no) ?   $certi_ib->app_no  :  ''  }}    
    ...
    @if ($PayIn->conditional_type == 1)  <!-- เรียกเก็บค่าธรรมเนียม  --> 
        ...
        @if (!is_null($PayIn->FileAttachPayInOne1To))
                <p> ค่าบริการในการตรวจประเมิน :
                    <a href="{{url('certify/check/file_ib_client/'.$PayIn->FileAttachPayInOne1To->file.'/'.( !empty($PayIn->FileAttachPayInOne1To->file_client_name) ? $PayIn->FileAttachPayInOne1To->file_client_name :  basename($PayIn->FileAttachPayInOne1To->file)  ))}}" 
    ...
    @endif
    <p style="text-indent: 50px;"> ทั้งนี้    {{  !empty($certi_ib->name) ?  $certi_ib->name  :  ''  }}  ต้องชำระค่าบริการในการตรวจประเมินภายใน 30 วัน นับจากวันที่ตรวจประเมินแล้วเสร็จ</p>

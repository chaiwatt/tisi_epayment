...
@endpush
    ...
    <div class="col-12">
        <button type="button" class="btn btn-success glow mr-1 mb-1 pull-right" data-toggle="modal" data-target="#AddUserForm" id="ButtonAddUserForm"> <i class="icon-plus" aria-hidden="true"></i>
            <span class="align-middle ml-25"> เพิ่ม</span>
        </button>
        <br>

        <h4>รายชื่อเจ้าหน้าที่ในกลุ่มงาน</h4>
    ...
    @include ('bsection5.workgroup.modal-add-user')

    <div class="col-12">
        <button type="button" class="btn btn-success glow mr-1 mb-1 pull-right" data-toggle="modal" data-target="#AddStdForm" id="ButtonAddStdForm"> <i class="icon-plus" aria-hidden="true"></i>
            <span class="align-middle ml-25"> เพิ่ม</span>
        </button>
        <br>

        <h4 >รายชื่อ มอก. ที่ดูแล</h4>
        <div class="table-responsive">
            <table class="table color-bordered-table primary-bordered-table" width="60%"id="div_table_scope">
                <thead>
                    <tr>
                ...
                <tbody class="font-medium-1" id="tbody-add-std">
                    @isset($workgroup_tis)
                        @foreach ( $workgroup_tis as $key=> $item_tis )
                        @php
                            $tis_standards = $item_tis->tis;
                        @endphp
                            <tr>
                                <td class="text-center">{{$key  +1}}</td>
                                <td>
                                    @if(!is_null($tis_standards))
                                        ...
                                        <input type="hidden" name="tis_id[]" value="{!! $item_tis->tis_id !!}">
                                    @endif
                                </td>
                                <td class="text-center font-medium-1">
                                    <button type="button" class="btn btn-icon rounded-circle btn-danger mr-1 mb-1 btn_remove_branch "><i class="fa fa-trash"></i></i></button>
    ...
    @include ('bsection5.workgroup.modal-add-std')
...
<div class="form-group">
    <div class="col-md-offset-4 col-md-4">
        ...
        @can('view-'.str_slug('bsection5-workgroup'))
            <a class="btn btn-default" href="{{url('/bsection5/workgroup')}}">

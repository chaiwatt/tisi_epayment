      ...
      <div class="col-sm-12">
          <div class="white-box">
              ...
              <div class="pull-right">
                  ...
                  @can('edit-'.str_slug('bsection5-workgroup-ib'))

                      <button class="btn btn-success btn-outline btn-sm  waves-effect waves-light" type="button" name="publish" id="publish"><span class="btn-label"><i class="fa fa-check"></i></span>เปิด</button>
                      <button class="btn btn-danger btn-outline btn-sm  waves-effect waves-light" type="button" name="no_publish" id="no_publish"><span class="btn-label"><i class="fa fa-close"></i></span>ปิด</button>

                  @endcan

                  @can('add-'.str_slug('bsection5-workgroup-ib'))
                      <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/bsection5/workgroup-ib/create') }}">
                          <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                      </a>
                  @endcan

                  @can('delete-'.str_slug('bsection5-workgroup-ib'))
    ...
    <script>
        ...
        $(function () {
            ...
            var table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/bsection5/workgroup-ib/data_list') !!}',
                    data: function (d) {
            ...
            $('#myTable tbody').on('change', '.js-switch', function(){
                ...
                if ($(this).is(":checked")) {
                    $.ajax({
                        method: "POST",
                        url: "{{ url('bsection5/workgroup-ib/update_status') }}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_status": id_status,
                            "state":1
            ...
            $(document).on('click', '#bulk_delete', function(){
                ...
                if(id.length > 0){
                    ...
                    if(confirm("ยืนยันการลบข้อมูล " + id.length + " แถว นี้ ?")){
                        $.ajax({
                            type:"POST",
                            url:  "{{ url('/bsection5/workgroup-ib/delete') }}",
                            data:{
                                _token: "{{ csrf_token() }}",
                                id: id
                            },
            ...
            $('#publish, #no_publish').on('click', function () {
                    ...
                    if (arrRowId.length > 0) {
                        ...
                        if (confirm("ยืนยันการ"+text_alert+" " + arrRowId.length + " แถว นี้ ?")) {
                            ...
                            $.ajax({
                                method: "POST",
                                url: "{{ url('/bsection5/workgroup-ib/update_publish') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "id_publish": arrRowId,
                                    "state": state

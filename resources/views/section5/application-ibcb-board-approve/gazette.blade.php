...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">บันทึกประกาศราชกิจจานุเบกษา IB/CB #{{ $applicationibcb->id }}</h3>
                    ...
                    {!! Form::model($applicationibcb, [
                        'method' => 'PATCH',
                        'url' => ['/section5/application-ibcb-board-approve/gazette-save', $applicationibcb->id],
...
@push('js')
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            //ปิดฟอร์ม ผลเสนอคณะอนุกรรมการ กมอ
            $('#box-approve-tisi').find('button[type="submit"]').closest('.row').remove();//กรอบบันทึก/ยกเลิก
            $('#box-approve-tisi').find('button[type="button"], a[title="ลบไฟล์"]').remove();//ปุ่มเพิ่ม/ลบ ไฟล์
            $('#box-approve-tisi').find('input[type="radio"]:not(:checked)').prop('disabled', true);
            $('#box-approve-tisi').find('input[type="text"], input[type="file"], textarea').prop('disabled', true);
